...
class PanelManagerProvider extends PanelProvider
{
    public function panel(Panel $panel): Panel
    {
        return $panel
            ->id('manager')
            ->path('') // Serve at the root del dominio de Manager (ver config/genesis.php)
            ->domain(parse_url(config('genesis.urls.manager'), PHP_URL_HOST))
            ->login()
            ->colors([
