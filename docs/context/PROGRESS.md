...
- **Bug real**: ninguno de los 2 `canView()` chequeaba ninguna Policy — solo verificaban que hubiera un tenant activo. Cualquier rol con acceso al Dashboard (incluido `Author`/Redactor, sin acceso a Contactos en la matriz de ADR-078) veía datos reales de leads/contactos del tenant.
- **Fix, con el criterio explícito del Tech Lead (exige AMBOS accesos, no solo uno)**: `canView()` de ambos widgets ahora es `$user->can('viewAny', Contact::class) && $user->can('viewAny', Form::class)` — delega a `ContactPolicy`/`FormPolicy` en vez de repetir listas de roles a mano (se mantiene sincronizado solo si cualquiera de las 2 matrices cambia a futuro). Con la matriz de 5 roles vigente, esto oculta ambos widgets a `Author` (sin ninguno de los 2) Y a `Editor` (tiene Contactos pero no Formularios) — `Editor` sigue teniendo su acceso normal a `ContactResource`, esto solo oculta el RESUMEN del Dashboard.
- **Archivos:** `app/Filament/Widgets/LeadsOverviewWidget.php`, `app/Filament/Widgets/RecentContactsWidget.php`.
- **Sin runtime de PHP en este sandbox** — balance verificado manualmente (OK).
- **Pendiente:** confirmación en vivo con cuentas `Editor`/`Author`/`Marketing` de que el Escritorio ya no muestra estos 2 widgets quien no debe verlos.

## 2026-09-18 — 2do fix real (mismo hilo): 404 de ruta no coincidente seguía sin redirigir — faltaba un `Route::fallback()` con sesión disponible

- **El Tech Lead probó el fix anterior (403→redirect) y reportó, con captura, que una URL de Studio que no matchea NINGUNA ruta** (`/cica360/preferencias`, con "c" — el slug real de la página es en inglés, `/cica360/preferences`) seguía mostrando un "404 | Not Found" crudo sin redirigir, pese a que YA existía un handler de `NotFoundHttpException` en `bootstrap/app.php` pensado exactamente para este caso.
- **Causa raíz real, distinta a la del 403**: ese handler de 404 nunca puede ver una sesión/usuario autenticado cuando la URL no matchea ninguna ruta registrada — el middleware del panel `cms` (`StartSession`, `FilamentAuthenticate`, etc.) está atado a LAS RUTAS que Filament registra; si el router no encuentra ninguna ruta que matchee, esos middlewares nunca corren (no hay ruta a la que atarlos), así que no hay sesión iniciada y `auth()->user()` dentro del exception handler siempre da `null` — el 403 SÍ funciona (esa ruta matchea, sus middlewares sí corren, `canAccess()` es lo que falla), pero el 404 de "ruta inexistente" es un caso estructuralmente distinto.
- **Fix real: `Route::fallback()` en `routes/web.php`**, scoped al dominio de Studio. Un `Route::fallback()` SÍ es una ruta real (aunque capture cualquier path no matcheado) — al vivir en `routes/web.php`, Laravel le aplica el grupo de middleware `web` (`StartSession` incluido) automáticamente, así que acá adentro la sesión SÍ está disponible y `auth()->user()` funciona. Redirige a un usuario autenticado a su Escritorio (o a Platform si es Super Admin sin tenant); un guest va a `Panel::getLoginUrl()`.
- **Detalle encontrado de paso, corregido antes de que fuera un problema**: estuve por copiar `route('login')` (el mismo patrón que usa `redirectGuestsTo()` en `bootstrap/app.php`) pero esa ruta NO EXISTE con ese nombre literal en esta app — confirmado leyendo el propio comentario de ese archivo ("esta app headless no tiene... una ruta web nombrada `login`") y el código fuente de Filament (`vendor/filament/filament/routes/web.php`): la ruta real se llama `filament.{panelId}.auth.login`. En `redirectGuestsTo()` ese `route('login')` es código goloso pero probablemente nunca se ejecuta en la práctica (Filament intercepta antes con su propio middleware de auth) — en el `Route::fallback()` nuevo SÍ se ejecutaría siempre para un guest, así que se usa `Panel::getLoginUrl()` (método público de Filament) en vez de hardcodear un nombre de ruta que no existe.
- **Archivos:** `routes/web.php`.
- **Sin runtime de PHP en este sandbox** — balance verificado manualmente (OK).
- **Pendiente:** confirmación en vivo del Tech Lead — visitar una URL de Studio inexistente logueado con distintos roles y como guest, confirmar que redirige en los 3 casos (usuario con tenant, Super Admin sin tenant, guest).

## 2026-09-18 — Fix real: link "Preferencias" visible sin acceso en el dropdown de perfil + 403 crudo de Laravel en vez de redirigir

- **3 síntomas reportados por el Tech Lead, con captura**, probando con una cuenta `Marketing` recién creada: (1) el dropdown del avatar mostraba el link "Preferencias" aunque ese rol no tiene acceso (ADR-078: `Preferences::canAccess()` = `[Admin, Soporte]` desde la expansión de roles) — clic → 403; (2) navegar por URL directa a una ruta sin acceso muestra la página de error CRUDA de Laravel/Symfony ("403 | Forbidden", sin estilo ni navegación de Filament) en vez de redirigir a algún lugar útil; (3) un "parpadeo" de esa misma pantalla en blanco con 403 durante el logout, "sea cual sea el rol", antes de aterrizar en el login.
- **Fix 1 — `MenuItem::make()->label('Preferencias')` en `PanelCmsProvider.php`**: no tenía `->visible()` (a diferencia de "Ir a Platform Manager", que sí lo tenía). Un `MenuItem` del dropdown de perfil no pasa por ninguna Policy/`canAccess()` automático de Filament — hay que gatearlo a mano. Se agrega `->visible(fn (): bool => Preferences::canAccess())`.
- **Fix 2 — `bootstrap/app.php`**: nuevo `$exceptions->render(function (AccessDeniedHttpException $e, Request $request) {...})`, mismo patrón/ubicación que el handler ya existente para `NotFoundHttpException|ModelNotFoundException` (404/tenant inválido) justo arriba. Para un usuario autenticado en el host de Studio, un 403 ahora redirige a su propio Escritorio (`$cmsPanel->getUrl($user->tenant)`) o a Platform si es Super Admin sin tenant — en vez de mostrarle la página de error sin estilo. No afecta la API (su propio envelope JSON, más abajo en el mismo archivo, sigue intacto) ni a un guest sin sesión (cae al comportamiento default).
- **Síntoma 3 (flicker en logout) no reproducido de forma aislada** — no hay runtime de PHP/navegador real en este sandbox para trazar el request/response exacto del logout. Revisados `EnsureUserAccessesOwnTenant`/`EnsurePasswordIsNotExpired`/`SyncTenantManagerWithFilament` (los 3 middlewares que corren en toda ruta autenticada de Studio, incluido logout) sin encontrar una rama de código que dispare un 403 ahí — los 3 dejan pasar explícitamente las rutas de logout. Es razonable esperar que el fix 2 de arriba (cualquier 403 en Studio para un usuario autenticado ahora redirige en vez de pintar la página cruda) elimine también este síntoma si su causa real era un 403 transitorio en algún punto de la cadena de logout — **pendiente de confirmación visual del Tech Lead tras desplegar**, no se puede dar por cerrado sin eso.
- **Archivos:** `app/Providers/Filament/PanelCmsProvider.php`, `bootstrap/app.php`.
- **Sin runtime de PHP en este sandbox** — balance verificado manualmente (OK) en ambos archivos. Sin tests nuevos (la lógica vive en un closure de `$exceptions->render()`, cubrirla bien requeriría un test HTTP real contra el host de Studio — no viable de escribir a ciegas sin poder correrlo).
- **Pendiente:** `php artisan test --compact`, `vendor/bin/pint --dirty --format agent`, y sobre todo confirmación en vivo del Tech Lead de los 3 síntomas — especialmente el logout, no verificado por causa raíz concreta acá.
...
- **4 rondas de `AskUserQuestion`** para resolver una ambigüedad real: la descripción inicial de `Marketing` y el `Editor` ya existente se solapaban casi por completo. Se confirmó que son 5 roles distintos, que `Marketing` puede personalizar Formularios (Admin+Soporte+Marketing), que `Author` no gana Contactos, y finalmente que la diferencia real es **"Editor SIN Multimedia/Menús/Sliders"** — `Marketing` conserva todo ese grupo, `Editor` lo pierde (se acota a Contenidos/Blog/Servicios/Testimonios + Contactos, sin retirarle Contactos como pedido explícito aparte).
- **Matriz final**: **Admin** sin cambios. **Soporte** = alcance técnico que tenía `Editor` en ADR-078 original (Desarrolladores/Preferencias/Formularios/Multimedia/Menús/Sliders/Contactos), nunca `User`. **Marketing** = Contenidos/Multimedia/Menús/Sliders/Testimonios + Contactos + Formularios, sin Desarrolladores/Ajustes/Preferencias/Usuarios. **Editor** = Contenidos/Blog/Servicios/Testimonios + Contactos únicamente (pierde Multimedia/Menús/Sliders/Formularios/Desarrolladores/Preferencias que tenía antes). **Author** sin cambios.
- **Implementación**: `UserRoleEnum` +2 casos. `TenantRolePolicy` (base, Contenidos) extiende sus 4 arrays a los 5 roles. `EditorManagedPolicy` renombrada a `SupportManagedPolicy` (el nombre viejo ya no describía la realidad — `Editor` salió del grupo) con `[Admin, Soporte, Marketing]`, usada por `MediaPolicy`/`MenuPolicy`/`SliderPolicy`/`FormPolicy`. `ContactPolicy` deja de extender ese grupo — array propio `[Admin, Soporte, Marketing, Editor]` (única Policy "técnica" donde Editor conserva acceso, a propósito). `UserPolicy` sin cambios. Pages de Desarrolladores/Preferencias (`ApiTokens`/`ApiPlayground`/`ApiDocumentation`/`Preferences`) pasan de `[Admin, Editor]` a `[Admin, Soporte]`. `PlanUsageWidget` gana `canView()` (reusa el trait `RestrictsPageToRoles` sobre una clase `Widget`, no solo `Page`) con `[Admin, Soporte]`. `UserResource.php`: `Radio::make('role')` y badge de la tabla ganan las 2 entradas nuevas.
- **Archivos:** `app/Enums/UserRoleEnum.php`; `app/Policies/{TenantRolePolicy,ContactPolicy,MediaPolicy,MenuPolicy,SliderPolicy,FormPolicy}.php`; `app/Policies/SupportManagedPolicy.php` (nuevo, reemplaza `EditorManagedPolicy.php`, borrado); `app/Filament/Pages/{ApiTokens,ApiPlayground,ApiDocumentation,Preferences}.php`; `app/Filament/Widgets/PlanUsageWidget.php`; `app/Filament/Resources/UserResource.php`; `docs/context/DECISIONS.md` (2do addendum ADR-078).
- **Sin runtime de PHP en este sandbox** — balance de llaves/paréntesis/corchetes verificado manualmente (script Python) en los 14 archivos tocados/nuevos. Grep confirmó cero referencias vivas a `EditorManagedPolicy` fuera de docs históricas; los tests existentes (`UserResourceTenantLimitTest`) solo usan los valores `Editor`/`Author` del enum, que siguen existiendo — no requieren cambios.
- **Pendiente:** `vendor/bin/pint --dirty --format agent`, `php artisan test --compact`, y sobre todo confirmación en vivo del Tech Lead con cuentas reales de los 5 roles — el punto más sensible es confirmar que `Editor` ya NO ve Multimedia/Menús/Sliders/Formularios pero SÍ sigue viendo Contactos, y que `PlanUsageWidget` desaparece del Dashboard para `Marketing`/`Editor`/`Author`. Commit pendiente.
...
- **Matriz de acceso acordada con el Tech Lead** (vía `AskUserQuestion`, 3 preguntas): **Admin** = control total (sin cambios). **Editor** = Contenidos/Blog/Servicios/Multimedia/Menús/Sliders/Testimonios/Contactos + grupo Desarrolladores/Preferencias, NUNCA gestión de Usuarios. **Author** (Redactor) = solo Page/Post/Service/Testimonial, sin borrar, sin publicar/activar, sin acceso a Menús/Sliders/Multimedia/Formularios/Contactos/Usuarios/Desarrolladores.
- **Patrón de implementación**, pedido explícito del Tech Lead ("que cada archivo policy hace un extend del policy master o base y no se repite el código en cada policy"): jerarquía de 2 niveles — `TenantRolePolicy` (base abstracta, genérica por 4 arrays de roles + chequeo de tenant) → `EditorManagedPolicy` (fija roles a `[Admin, Editor]`) → Policies concretas por Resource, la mayoría vacías (`PagePolicy`/`PostPolicy`/`ServicePolicy`/`TestimonialPolicy` extienden la base; `MenuPolicy`/`SliderPolicy`/`MediaPolicy`/`FormPolicy`/`ContactPolicy` extienden `EditorManagedPolicy`; `UserPolicy` extiende la base directo con `[Admin]` únicamente en los 4 arrays — la Policy más estricta).
- **`canAccess()` explícito en 4 Pages sin modelo Eloquent** (`ApiTokens`/`ApiPlayground`/`ApiDocumentation`/`Preferences`) — confirmado leyendo el vendor de Filament que una Page "custom" NO delega a ninguna Policy por default (siempre `true`), a diferencia de un Resource (`canAccess()` sí delega a `canViewAny()` → Policy, así que `MediaResource` compone solo con la nueva `MediaPolicy` sin tocarlo). Trait nuevo `RestrictsPageToRoles` para no repetir el fallback "sin rol = Admin" en las 4 Pages.
- **Bloqueo de publicar/activar para `Author`**: trait nuevo `LocksPublishingForAuthor`. `status`+`published_at` con `->disabled()->dehydrated()` en `PageResource`/`PostResource`/`ServiceResource` (el `dehydrated()` explícito evita que el campo deshabilitado pierda su valor por defecto al crear). En `TestimonialResource` (usa `is_visible` en vez de `status`) se bloquean las 3 superficies que lo tocan: `Toggle` del form, `ToggleColumn` editable inline de la tabla, y los 2 `BulkAction` "Marcar como visibles/ocultos" (`->visible()`, para que ni aparezcan en el menú de un Author).
- **Sin runtime de PHP en este sandbox** — balance de llaves/paréntesis/corchetes verificado manualmente (script Python) en los 15 archivos nuevos/tocados. Pendiente real: `vendor/bin/pint --dirty --format agent`, `php artisan test --compact`, y sobre todo probar en vivo en Studio con una cuenta `Editor` y una `Author` reales.
- **Archivos:** `app/Http/Middleware/SyncTenantManagerWithFilament.php`; `app/Policies/{TenantRolePolicy,EditorManagedPolicy,PagePolicy,PostPolicy,ServicePolicy,TestimonialPolicy,MenuPolicy,SliderPolicy,MediaPolicy,FormPolicy,ContactPolicy,UserPolicy}.php`; `app/Filament/Concerns/{RestrictsPageToRoles,LocksPublishingForAuthor}.php`; `app/Filament/Pages/{ApiTokens,ApiPlayground,ApiDocumentation,Preferences}.php`; `app/Filament/Resources/{PageResource,PostResource,ServiceResource,TestimonialResource}.php`; `docs/context/DECISIONS.md` (ADR-078).
- **Pendiente futuro, NO implementado esta vuelta** (documentado en ADR-078, sin diseño): `filament-shield` para planes de pago (roles/permisos configurables por el tenant), Auspicio con roles preset + posibilidad de crear roles custom, tests automatizados de la matriz de acceso.
...
- **`docs/api/v1.md`:** sacadas las 8 anotaciones de fecha encontradas — `### GET /services (agregado 2026-09-02)`, `### GET /services/{slug} (agregado 2026-09-02)`, `### GET /testimonials (agregado 2026-09-02)`, `**footer` (2026-09-15, solo en...)`, `**image_detail` (2026-09-14, solo en...)`, `**Excepción (2026-09-14):**`, el paréntesis completo de "Ejemplo con submenú anidado (agregado 2026-09-02 — antes el ejemplo solo mostraba...)" y `` `type` (agregado 2026-09-02) ``. Se conservó en cada caso la info técnica real (ej. "solo en `GET /services/{slug}`, NO en `GET /services`"), solo se sacó la fecha/el marco de "esto se agregó". De paso: la referencia interna `(MenuTreeBuilder, ver ADR-045)` en la sección de menús (un link a un ADR interno que se había colado después del barrido de limpieza de una sesión anterior) y la frase "Hasta ahora `testimonials` solo se consumía..." (mismo problema, framing histórico) también se reescribieron a presente/neutro. También se sacó "· Última actualización: 2026-09-18" del encabezado del documento (línea 3) — mismo criterio.
- **`docs/api/openapi.v1.yaml`:** mismas 2 anotaciones equivalentes en la descripción del schema `Service` (`2026-09-14: imagen secundaria...`, `2026-09-15: footer dinámico...`, `Excepción (2026-09-14): why_choose_us.text...`) — sacadas las fechas, texto técnico intacto.
- **Verificación:** grep de `[0-9]{4}-[0-9]{2}-[0-9]{2}` sobre ambos archivos — solo quedan las fechas legítimas dentro de valores de ejemplo JSON (`"published_at": "2026-08-16T..."`), que ilustran el formato de fecha del API, no anotaciones de changelog. YAML validado con `yaml.safe_load()` (OK).
- **Archivos:** `docs/api/v1.md`, `docs/api/openapi.v1.yaml`.
- **Pendiente:** sumar al deploy pendiente.

## 2026-09-18 — Todos los ejemplos de contenido de `docs/api/v1.md`/`openapi.v1.yaml` pasan a genéricos (sin rubro seguros/CICA360) + mensajes de error de `openapi.v1.yaml` reconciliados con `bootstrap/app.php`

- **Pedido del Tech Lead**, viendo el JSON de ejemplo de `GET /testimonials` con la quote real de CICA360 ("El acompañamiento de CICA360 fue clave para elegir la cobertura correcta."): "en el demo no puede haber nada de otro tenant como ejemplo... deja como ejemplo generico". Aunque la personalización del `{tenant_slug}` de la entrada anterior ya resolvía las URLs, TODO el contenido de ejemplo dentro de los JSON (`seo_title`, `quote`, `title`, `subtitle`, `intro`, etc.) seguía siendo el contenido REAL sembrado para CICA360 — un tenant de otro rubro (no seguros/consultoría) podía confundirse pensando que el producto es vertical-específico.
- **`docs/api/v1.md`:** reescritos a placeholders neutros ("Servicio de ejemplo", "Título de ejemplo del artículo", "Ejemplo de testimonio: un comentario breve sobre la experiencia con el servicio.", etc.) todos los valores de ejemplo en los JSON de `GET /pages/{slug}` (meta SEO, cta), el item de `services_grid`, `GET /posts`/`GET /posts/{slug}` (slug/title/subtitle/excerpt/content/meta), `GET /services`/`GET /services/{slug}` (slug/title/subtitle/content.intro/offers/coverages/why_choose_us/tip), `GET /testimonials` (quote), el árbol de `GET /menus/{slug}` (títulos/hrefs de servicio y link externo), `GET /sliders/{slug}` (title/subtitle/label del link), `GET /media/{uuid}` (alt_text) y el request/placeholder de `POST /forms/{slug}/submit`/`GET /forms/{slug}`. Se dejaron intactos los campos que SÍ son defaults reales de la plataforma (ej. `thank_you.title/description/alert_*` — el fallback hardcodeado de `FormResource.php`, genérico por diseño desde antes de esta sesión) y estructurales sin connotación de rubro (`"countries": [{"iso": "UY", "name": "Uruguay"}]`, nombres de persona genéricos como "María Fernández"/"Juan Pérez").
- **2 menciones de "CICA360" en prosa** (citándola como ejemplo de "sitio estático en shared hosting" y de "frontend desplegado por CI/CD") también se genericizaron a "un sitio estático" / "un sitio estático desplegado por CI/CD" — el Tech Lead pidió "nada de otro tenant como ejemplo", sin acotarlo solo a JSON.
- **`app/Filament/Pages/ApiPlayground.php`:** el preset `posts.show` traía hardcodeado un slug real de CICA360 (`posts/como-elegir-seguro-de-vida`) — el único preset funcional (no solo documentación) que filtraba contenido de otro tenant/rubro. Cambiado al mismo patrón placeholder que ya usaba `services.show` (`posts/{slug}`).
- **`docs/api/openapi.v1.yaml`:** el `default` de la variable `tenant_slug` en ambos `servers` (Producción/Desarrollo local) era `cica360` — a diferencia de `docs/api/v1.md` (que SÍ se personaliza dinámicamente por tenant vía `ApiDocumentation::readMarkdown()`), este YAML se sirve crudo sin ninguna sustitución (`routes/web.php`, ruta `docs.openapi-yaml`), así que cualquiera que lo abriera en Swagger UI veía el tenant de otra cuenta precargado. Cambiado a `tu-tenant`, un placeholder inequívoco. También se genericizó el `message` de ejemplo del request body de `POST /forms/{slug}/submit`.
- **Hallazgo adicional en el mismo repaso, no relacionado al pedido de contenido genérico pero real:** los 4 `examples`/`example` de las respuestas de error (`401` sin token, `403`, `422`, `429`) en `openapi.v1.yaml` habían quedado en voseo rioplatense y con texto desactualizado ("No tenés permiso...", "Revisá los datos...", "Intentá más tarde...", "Enviá un token Bearer...") — el barrido de voseo de esta misma sesión había tocado `docs/api/v1.md` y `bootstrap/app.php`, pero se había pasado por alto este archivo YAML. Reconciliados los 4 mensajes para que coincidan EXACTO con los que devuelve `bootstrap/app.php` (mismo criterio ya aplicado en `v1.md`).
- **Corrección de seguimiento en el mismo turno:** el Tech Lead notó que el ejemplo de `GET /media/{uuid}` seguía mal: `"url": "https://cdn.stamless.com/cica360/logo.png"`. No era solo un problema de genericidad — el dominio y la forma del path eran directamente FICTICIOS. Se investigó el código real (`app/Models/Media.php::url()`, `config/filesystems.php`, `app/Filament/Schemas/MediaUpload.php`, `.env.example`) y se confirmó: (a) el dominio público real viene de `R2_URL` (ej. `https://pub-<hash>.r2.dev`), nunca `cdn.stamless.com`; (b) el slug del tenant NO es un segmento de path — va como prefijo del nombre de archivo dentro del directorio fijo `media/` (`media/{tenant_slug}_media_{YmdHis}.{ext}`), sin carpeta por tenant. Corregido a `"url": "https://media.stamless.com/media/ejemplo_media_20260101120000.png"` — `media.stamless.com` es el dominio custom real ya confirmado en producción (mapeado al bucket R2 vía `R2_URL`, ver entrada "3er fix real" del 2026-09-18 más abajo, donde el Tech Lead lo confirmó por tinker), y el nombre de archivo queda genérico (no un dato de una cuenta real). Esto también corrige la entrada de PROGRESS.md de más arriba (mismo día), que había asumido —incorrectamente— que la URL real de CDN sí incluía el tenant como carpeta.
- **Verificación:** `grep` de `CICA360|cica360|seguro|cobertura|póliza` sobre ambos archivos de `docs/api/` sin resultados tras el cambio (salvo el `cica360` legítimo que queda dinámico en `v1.md`, sustituido en runtime), y `grep` de `cdn.stamless` sin resultados en todo el repo tras el fix. Balance de llaves/paréntesis/corchetes verificado en `ApiPlayground.php` (OK).
- **Archivos:** `docs/api/v1.md`, `docs/api/openapi.v1.yaml`, `app/Filament/Pages/ApiPlayground.php`.
- **Pendiente:** sumar al deploy pendiente (`./deploy.sh` + `./production.sh`).

## 2026-09-18 — "API Documentation"/"API Playground" personalizan el tenant slug de ejemplo por el tenant ACTUAL de Console (antes: `cica360` fijo, sin importar desde qué cuenta se abriera)

- **Pregunta del Tech Lead**, mirando la doc renderizada: "la documentacion personaliza por tenant los ejemplos verdad? por que casi en todo veo cica360... si estas personzalizando desde el tenant que se abra la documentacion cambiará esos nombres". Respuesta encontrada al leer el código: NO, no estaba personalizado — `ApiDocumentation::readMarkdown()` hacía `file_get_contents()` crudo del `.md` sin ninguna sustitución, y `ApiPlayground::mount()` hardcodeaba `'tenant_slug' => 'cica360'` como valor inicial del campo, sin leer el tenant real de Console. Cualquier tenant que no fuera CICA360 vería siempre `cica360` en los ejemplos de URL/curl, aunque su propio slug fuera otro — bug real de una plataforma que es multi-tenant de punta a punta.
- **Fix en `ApiDocumentation.php`:** `readMarkdown()` ahora hace `str_replace('cica360', $tenantSlug, $raw)` sobre el markdown crudo ANTES de convertirlo a HTML, usando `Filament::getTenant()?->slug` del tenant actual de Console — el `str_replace` es case-sensitive a propósito: solo reemplaza las instancias en MINÚSCULA de `cica360` (usadas exclusivamente como segmento `{tenant_slug}` en URLs/curl/fetch de ejemplo a lo largo del documento), dejando intactas las menciones en MAYÚSCULA (`CICA360`, la marca) que aparecen en prosa o en valores de ejemplo dentro de JSON (`seo_title`, `quote`, etc.) — esas SÍ son contenido ilustrativo real de esa cuenta puntual, no parte de la ruta del API, y está bien que se mantengan como ejemplo aunque se esté viendo la doc desde otro tenant.
- **Fix en `ApiPlayground.php`:** `mount()` ahora precarga `tenant_slug` con `Filament::getTenant()?->slug` (fallback a `'cica360'` solo si por algún motivo no hay tenant resuelto, defensivo) — el campo sigue siendo editable a mano por si alguien quiere apuntar el playground a otro tenant a propósito, pero el valor inicial ahora es correcto para quien sea que abra la página.
- **Ajuste de redacción en `docs/api/v1.md`:** la línea "Por ejemplo, contra CICA360: `.../v1/cica360/pages`." quedaba contradictoria tras la sustitución (diría "contra CICA360" pero mostraría el slug de otro tenant) — reescrita a "Por ejemplo, para este tenant: `.../v1/cica360/pages`.", que lee bien sin importar qué slug termine sustituido.
- **Efecto colateral, luego corregido:** en su momento se asumió que el ejemplo de `GET /media/{uuid}` (`"url": "https://cdn.stamless.com/cica360/logo.png"`) también quedaba "más preciso" al personalizarse el slug. Esto era incorrecto — ver la entrada de más abajo (mismo día) donde se corrige: ni el dominio (`cdn.stamless.com`) ni la forma del path (`/{tenant_slug}/archivo`) reflejan la estructura real de R2 (`Media::url()` delega en el disco `public`/R2 vía `R2_URL`, y el slug del tenant va como PREFIJO DEL NOMBRE DE ARCHIVO dentro de `media/`, no como carpeta — ver `app/Filament/Schemas/MediaUpload.php`).
- **Verificación:** balance de llaves/paréntesis/corchetes verificado vía script Python en los 2 archivos PHP tocados (OK). Sin runtime de PHP en este sandbox para probar en vivo que `Filament::getTenant()` resuelve correctamente dentro de `ApiDocumentation`/`ApiPlayground` (ambas son pages dentro del panel `cms`, tenant-scoped por diseño — mismo mecanismo que ya usa el resto de Console, ej. `ApiTokens::activeTokensCountForTenant()`).
- **Archivos:** `app/Filament/Pages/ApiDocumentation.php`, `app/Filament/Pages/ApiPlayground.php`, `docs/api/v1.md`.
- **Pendiente:** confirmar visualmente en Studio, idealmente entrando como un tenant que NO sea CICA360, que la doc y el playground ya muestran el slug correcto. Sumar a la lista de deploy pendiente (`./deploy.sh` + `./production.sh`).
...
- **Causa real, nada que ver con el contenido editado:** `App\Filament\Pages\ApiDocumentation::readMarkdown()` lee `base_path('docs/api/v1.md')` en cada request (por diseño — ADR-018, "fuente de verdad única, no duplica contenido"), y la ruta `docs.openapi-yaml` (`routes/web.php`) sirve `base_path('docs/api/openapi.v1.yaml')` crudo. `deploy.sh` (el script de sync a stage) tenía `--exclude='/docs/'` en su lista de exclusiones de `rsync` — pensado para no llevar al servidor la documentación interna de trabajo (`docs/context/`: `CURRENT_STATE.md`, `TASK.md`, `DECISIONS.md`, etc.), pero redactado de forma demasiado amplia: excluía la carpeta `docs/` COMPLETA, incluyendo `docs/api/`, que sí es una dependencia real de runtime de esas 2 páginas. Confirmado que el archivo SÍ existe y está actualizado en el repo (`git show origin/main:docs/api/v1.md` lo trae sin problema) — nunca se perdió contenido, el archivo simple y llanamente nunca llegó al servidor por ningún deploy que haya usado este script.
- **Alcance del bug:** no es nuevo de esta vuelta — cualquier stage/producción desplegado con `deploy.sh` desde que existe la página "API Documentation" (ADR-018) tendría este mismo error, sin relación con los cambios de contenido de hoy. `production.sh` (stage → prod) no tiene este problema — sincroniza todo lo que ya esté en stage sin excluir `docs/`, así que una vez corregido en `deploy.sh` y desplegado a stage, la promoción a producción lo arrastra sin cambios adicionales.
- **Fix:** `--exclude='/docs/'` → `--exclude='/docs/context/'` en `RSYNC_EXCLUDES` de `deploy.sh` — sigue sin llevar al servidor la documentación interna de trabajo (no es necesaria en runtime, y no debería quedar expuesta), pero `docs/api/` (con `v1.md` y `openapi.v1.yaml`) ahora sí se sincroniza.
- **Verificación:** cambio en un script bash, sin balance de llaves PHP que verificar. Revisión visual de la línea modificada (única línea cambiada en el array de exclusiones).
- **Archivos:** `deploy.sh`.
- **Pendiente:** correr `./deploy.sh` (stage) y confirmar en vivo que "API Documentation" y el link de `openapi.v1.yaml` ya cargan; después `./production.sh` para promoverlo. Sin ese deploy, la página sigue mostrando el error en cualquier ambiente ya desplegado, aunque el contenido en el repo ya esté correcto.

## 2026-09-18 — API Playground + API Documentation actualizados, y barrido de voseo rioplatense en toda la app (ADR-051 aplicado de fondo)

- **Pedido del Tech Lead:** revisar `ApiPlayground.php` por si faltaban endpoints nuevos, actualizar `docs/api/v1.md` (la fuente real de `studio.stamless.com/cica360/api-documentation`) con SMTP propio, despliegue automático con Git y otras novedades de seguridad/uso del API, y — con frustración explícita, "estas usando mucho: vos, andá, ejecutá, prolijo, etc" — eliminar el voseo rioplatense de todo lo que no sea la marca CICA360, en español neutro pero sin perder tono de buen copywriting/UX.
- **`ApiPlayground.php` (Console):** el array `PRESETS` ya incluía los 11 endpoints reales de `routes/api.php` (`pages`, `posts`, `services`, `testimonials`, `menus`, `sliders`, `media`, `forms.show`, `forms.submit`, `settings.tracking`) — no faltaba ningún endpoint funcional. Se corrigieron 4 strings en voseo (`$subheading`, 2 `helperText`, 1 `placeholder`) a español neutro.
- **`docs/api/v1.md` — contenido nuevo:**
  - `### GET /forms/{slug}` (no estaba documentado): shape completo de `FormResource` (campos dinámicos por `FormFieldTypeEnum`, incluyendo `tel_country`; objeto `thank_you` con placeholder `{name}`).
  - `### POST /forms/{slug}/submit` reescrito: ya no asume un set fijo de campos (ahora depende de `GET /forms/{slug}`), documenta los headers opcionales `X-Forwarded-For`/`X-Origin-Country`, la protección honeypot (`honeypot`/`_hp_check`/`_gotcha` → `201` simulado sin persistir) y que la respuesta trae `thank_you` resuelto sin request adicional.
  - `### GET /settings/tracking` (no estaba documentado): `meta_pixel_id`/`gtm_id`, ambos nullable.
  - `### Límites y alcance de los tokens` (nueva, bajo "Autenticación"): tope de tokens activos por plan (Free 5, Auspicio/Convenio 10), y el mecanismo `platform` (`Web`/`App`) + `allowed_origin` de `ValidateTokenOrigin` — no documentado hasta ahora pese a estar en producción desde ADR-059.
  - `### Cómo evitar exponer el token en el navegador` (nueva, dentro de "Buenas prácticas"): patrón de proxy server-side para sitios estáticos sin backend propio (el mismo que ya usa CICA360 con `contacto.php`), explicado en términos genéricos para cualquier tenant.
  - `## Funciones de Console relacionadas` (nueva, al final): **SMTP propio** (campos de Preferencias, comportamiento de "dejar vacío no borra" en la contraseña, botón "Probar conexión", fallback al remitente global si no se configura) y **Despliegue automático (Git)** (repositorio + token + toggle, debounce, independiente de tokens/abilities de la API pública) — ninguna de las dos es un endpoint, se documentan como features de Console porque son parte del flujo "un submit de formulario llega a destino" / "un contenido publicado se refleja en el sitio".
- **Reconciliación de mensajes de error:** la tabla de `## Errores` citaba texto ligeramente distinto al que realmente devuelve la API. Se corrigió la tabla para que coincida EXACTO con `bootstrap/app.php` (ver debajo) — cualquier developer que copie el ejemplo de la tabla ahora ve el mensaje real.
- **Voseo eliminado, alcance ampliado más allá de los 2 archivos nombrados** (el Tech Lead avisó "estas usando mucho", señal de que el problema era sistémico, no puntual — se grepeó todo `app/` con un set amplio de terminaciones voseo-rioplatenses):
  - `bootstrap/app.php` — **cambio de mayor impacto**: los 7 mensajes fijos del envelope de error de la API pública (401/403/404/422/429/500/default) reescritos a español neutro (ej. `"Se requiere un token Bearer en el header Authorization."`, `"No tiene permiso para acceder a este recurso."`, `"Revisar los datos enviados."`). **Este es texto real de producción, no solo copy de Console** — `errors.code` (el contrato estable, pensado para lógica de cliente) no cambió, solo `message` (documentado como "puede cambiar de texto sin previo aviso").
  - `app/Http/Controllers/Api/V1/FormSubmissionController.php` — mensaje `422` de campos faltantes, mismo criterio que arriba.
  - `app/Filament/Resources/UserResource.php` — 2 notificaciones (`No podés desactivar/eliminar tu propio usuario` → `No es posible desactivar/eliminar el propio usuario`) + 1 mensaje de límite de plan.
  - `app/Filament/Resources/MediaResource.php` — mensaje de límite de plan (`mejorá de plan` → `mejorar de plan`).
  - `app/Filament/Pages/ApiTokens.php` — 3 strings (`Copiá el token`/`Copiá el nuevo token` → `Copiar...`; `emptyStateDescription`).
  - `app/Filament/Resources/PageResource.php` — 1 `description()` del bloque de logos.
  - `app/Filament/Pages/ApiPlayground.php` — 4 strings, ver arriba.
- **Verificación:** sin runtime de PHP en este sandbox (no hay `php` disponible) — balance de llaves/paréntesis/corchetes verificado manualmente en los 7 archivos PHP tocados (todos OK; `PageResource.php` tiene un desbalance de paréntesis preexistente confirmado vía `git show HEAD`, no introducido por este cambio, no relacionado a las líneas editadas). Pendiente correr `vendor/bin/pint --dirty --format agent` en un entorno con PHP.
- **Archivos:** `docs/api/v1.md`, `bootstrap/app.php`, `app/Http/Controllers/Api/V1/FormSubmissionController.php`, `app/Filament/Resources/UserResource.php`, `app/Filament/Resources/MediaResource.php`, `app/Filament/Pages/ApiTokens.php`, `app/Filament/Resources/PageResource.php`, `app/Filament/Pages/ApiPlayground.php`.
- **Pendiente:** correr `pint`, evaluar si el cambio de mensajes en `bootstrap/app.php` amerita un ADR propio (es un cambio de contrato-adyacente de la API pública, aunque `errors.code` no cambió), commit, y confirmar con el Tech Lead que no haya integraciones externas haciendo `string match` sobre los mensajes de error viejos.
...
- **`config/filesystems.php`:** el disco `'public'` deja de ser un disco local fijo — ahora resuelve su propio `driver`/credenciales según `env('FILESYSTEM_DISK')`: local (`storage/app/public`, symlink de `storage:link`) si no es `'r2'`, o las mismas credenciales R2 que ya usaba el disco `'r2'` si `FILESYSTEM_DISK=r2`. Con el disco de media SIEMPRE llamado `'public'` (nunca `'r2'`), `ImageColumn::getVisibility()` infiere `'public'` correctamente solo (compara el NOMBRE del disco, no lee la config), sin overrides por código en ningún lado. Se agregó además un disco nuevo, `'local_public'` — FIJO, nunca condicional, siempre el filesystem local del servidor — para el código que necesita garantizado "el disco local de este server" sin importar el ambiente (ver abajo).
- **`MediaUpload::diskName()` y los 3 call sites duplicados en `MediaResource::form()`** simplificados de un ternario (`config('filesystems.default') === 'local' ? 'public' : config('filesystems.default', 'public')`) a simplemente `'public'` literal — el disco ya resuelve el switch local/R2 puertas adentro.
- **Migración de datos** (`2026_09_18_080000_normalize_media_disk_to_public`): `UPDATE media SET disk = 'public' WHERE disk IN ('r2', 'local', 's3')` — normaliza los registros `Media` YA guardados en producción con `disk = 'r2'` (todo lo subido antes de este fix), para que también queden alineados con el disco canónico. `down()` intencionalmente vacío (varios valores de origen colapsan al mismo destino, sin un único "estado anterior" real).
- **Regresión encontrada y corregida en el mismo turno:** el disco `'public'` condicional rompía 2 lugares que asumían que `'public'` era SIEMPRE el filesystem local — `App\Console\Commands\SyncMediaToR2Command` (el comando real `media:sync-r2`, muy probablemente al que se refería el Tech Lead al principio de este mismo hilo preguntando si había que "re-ejecutar el comando de sincronización con R2") escaneaba `Storage::disk('public')` para encontrar archivos locales a subir a R2 — en un servidor con `FILESYSTEM_DISK=r2`, eso habría escaneado el propio bucket R2 en vez del disco local, rompiendo el comando por completo. Mismo problema en `Cliente0MediaSeeder::seedFile()` (lee los assets sembrados del repo). Ambos reescritos para usar `'local_public'` (fijo) al LEER la fuente y `'public'` (dinámico) al ESCRIBIR/verificar el destino real; el comando además pasa a escribir siempre `MediaDiskEnum::Public` en la base (antes escribía el `--disk` literal, ej. `'r2'`) — mismo criterio canónico en todo el codebase. Su test (`SyncMediaToR2CommandTest`) actualizado para fakear `'local_public'` en vez de `'public'`, y la aserción de disco pasa de `MediaDiskEnum::R2` a `MediaDiskEnum::Public`.
- **Los 4 `ImageColumn` afectados** (`ServiceResource`, `TestimonialResource`, `PostResource`, `MediaResource`) pierden el `->visibility('public')` explícito del fix anterior — ya no hace falta, el disco resuelve correctamente solo. Se mantiene `->checkFileExistence(false)` (fix independiente, no relacionado al nombre del disco) y `->getStateUsing()` donde aplica.
- **Verificación:** sin runtime de PHP en este sandbox — balance de llaves/paréntesis/corchetes verificado manualmente en los 10 archivos tocados (OK).
- **Archivos:** `config/filesystems.php`, `app/Filament/Schemas/MediaUpload.php`, `app/Filament/Resources/MediaResource.php`, `app/Console/Commands/SyncMediaToR2Command.php`, `database/seeders/Cliente0MediaSeeder.php`, `database/migrations/2026_09_18_080000_normalize_media_disk_to_public.php` (nuevo), `tests/Feature/Console/SyncMediaToR2CommandTest.php`, `app/Filament/Resources/ServiceResource.php`, `app/Filament/Resources/TestimonialResource.php`, `app/Filament/Resources/PostResource.php`.
- **Pendiente:** `php artisan migrate` (corre la normalización de datos), `php artisan test --compact --filter=SyncMediaToR2CommandTest`, deploy (`deploy.sh` + `production.sh`) y confirmación visual final del Tech Lead en Servicios/Testimonios/Publicaciones/Biblioteca de Medios.

## 2026-09-18 — 3er fix real, mismo hilo: `ImageColumn` arma una URL FIRMADA de R2 en vez de la pública simple — `getVisibility()` infiere `'private'` cuando el disco no se llama literalmente `'public'`

- **El Tech Lead corrió el diagnóstico exacto que pedí:** `Storage::disk('r2')->url('media/cica360_media_header-tablet.webp')` en tinker de producción devolvió `https://media.stamless.com/media/cica360_media_header-tablet.webp` — URL pública simple, sin firma, confirmando que el disco/credenciales R2 están bien configurados. Pero inspeccionando el `<img>` roto DESPUÉS del 2do fix (`checkFileExistence(false)`), el `src` ya no estaba vacío — traía `https://stamless-storage.{account}.r2.cloudflarestorage.com/media/cica360_media_service_seguridad_financiera.webp?X-Amz-Content-Sha256=UNSIGNED-PAYLOAD&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=...&X-Amz-Signature=...` — una URL FIRMADA (presigned) contra el endpoint crudo de R2, ni remotamente parecida a la que arma `Storage::disk('r2')->url()`.
- **Causa raíz real, en el mismo archivo fuente de Filament** (`ImageColumn::getVisibility()`): `return ($this->getDiskName() === 'public') ? 'public' : 'private';` — la visibilidad se infiere COMPARANDO EL NOMBRE DEL DISCO contra el string literal `'public'`. Como el disco real de estas columnas es `'r2'` (no `'public'`), Filament asume que es privado — sin importar que `config/filesystems.php` tenga `'visibility' => 'public'` en la config del disco `r2`, o que `MediaUpload` suba los archivos con `->visibility('public')`. `ImageColumn` no consulta esa config, solo el nombre. Con `getVisibility() === 'private'`, `getImageUrl()` (ver 2do fix, mismo método) toma la rama `$storage->temporaryUrl(...)` en vez de `$storage->url(...)` — de ahí la URL firmada.
- **Por qué el formulario de edición siempre funcionó:** de nuevo, `MediaUpload::previewUrl()` no pasa por NINGUNA de estas dos inferencias de `ImageColumn` (ni `shouldCheckFileExistence()` ni `getVisibility()`) — llama `Storage::disk()->url()` directo, sin intermediarios.
- **Fix:** `->visibility('public')` explícito en las mismas 4 columnas del fix anterior (`ServiceResource`, `TestimonialResource`, `PostResource`, `MediaResource`) — fuerza `getVisibility()` a devolver `'public'` sin pasar por la inferencia del nombre de disco, y por lo tanto `getImageUrl()` toma la rama `$storage->url()` (la misma URL simple ya confirmada funcional por tinker).
- **Verificación:** sin runtime de PHP en este sandbox — balance de llaves/paréntesis/corchetes verificado manualmente en los 4 archivos (OK).
- **Archivos:** `app/Filament/Resources/ServiceResource.php`, `app/Filament/Resources/TestimonialResource.php`, `app/Filament/Resources/PostResource.php`, `app/Filament/Resources/MediaResource.php`.
- **Pendiente:** deploy (`deploy.sh` + `production.sh`) y confirmación en vivo. Con los 3 fixes acumulados del día (estado → existencia → visibilidad) ya no queda ninguna rama conocida de `ImageColumn`/Filament que pueda devolver una URL vacía o incorrecta para este caso — si tras este deploy sigue roto, el próximo paso sería revisar la URL pública real en el navegador (¿carga `media.stamless.com/...` directamente? ¿DNS/SSL de ese dominio están bien?).

## 2026-09-18 — 2do fix real, mismo hilo: `ImageColumn` seguía con `src` vacío tras el `getStateUsing()` — `getImageUrl()` de Filament hace un `exists()` extra contra R2 antes de armar la URL

- **El Tech Lead confirmó que ya había corrido el pipeline completo de deploy** (`deploy.sh` → stage, `production.sh` → prod, que ya limpia TODOS los caches de Laravel — `optimize:clear`/`config:cache`/`route:cache`/`view:cache`/`filament:optimize`) con el fix de `getStateUsing()` de la entrada anterior, y el thumbnail seguía exactamente igual de roto (misma captura, `src` vacío) — descartando timing de deploy y cacheo de Laravel como explicación.
- **Causa raíz real, encontrada en el código fuente de Filament** (`vendor/filament/tables/src/Columns/ImageColumn.php`, método `getImageUrl()`): ANTES de construir la URL final, si `shouldCheckFileExistence()` está activo (es el default, `true`), llama `$storage->exists($state)` — un `HeadObject` extra contra el disco (R2) por cada fila visible del listado. Si esa llamada devuelve `false` O tira `UnableToCheckFileExistence` (token de R2 con permisos acotados, latencia, lo que sea), el método corta ahí mismo y devuelve `null` — mismo síntoma visual (`src=""`) que el bug ya corregido del estado, pero por una causa completamente distinta. El `getStateUsing()` del fix anterior era necesario (sin él nunca se llegaba siquiera a intentar `getImageUrl()`) pero no suficiente — quedaba este segundo gate.
- **Por qué el formulario de edición siempre funcionó:** `MediaUpload::previewUrl()` (el mecanismo real detrás del preview en "Imágenes del servicio") NUNCA hizo este chequeo — arma la URL directo con `Storage::disk($media->disk?->value ?? 'public')->url((string) $media->path)`, confiando en que el registro `Media` de la base de datos representa un archivo real. Mismo criterio que `Media::url()` (usado por la API pública). El listado (`ImageColumn`) era el único lugar de la app que sí revalidaba contra el disco en cada render.
- **Fix:** `->checkFileExistence(false)` en las 4 columnas `ImageColumn` de la app que apuntan a `Media` (`ServiceResource`, `TestimonialResource`, `PostResource` — los 3 reportados/con el mismo patrón — y `MediaResource`, la Biblioteca de Medios, corregida preventivamente por compartir el mismo riesgo aunque no se reportó rota). Alinea el listado con el mismo criterio de confianza que ya usa el resto de la app, y de paso evita un `HeadObject` extra a R2 por cada fila visible de cada listado (mejora de performance, no solo de corrección).
- **Verificación:** sin runtime de PHP en este sandbox — balance de llaves/paréntesis/corchetes verificado manualmente en los 4 archivos (OK).
- **Archivos:** `app/Filament/Resources/ServiceResource.php`, `app/Filament/Resources/TestimonialResource.php`, `app/Filament/Resources/PostResource.php`, `app/Filament/Resources/MediaResource.php`.
- **Pendiente:** deploy (`deploy.sh` + `production.sh`) y confirmación en vivo del Tech Lead — esta vez el cambio no depende de ningún cache de Laravel (no es config/route/view), así que si sigue roto después de este deploy, el problema es otro (ej. credenciales/permisos reales del token de R2 en producción, verificable con `Storage::disk('r2')->url($path)` en tinker contra un `Media` real).
...
- **Nueva sección "Despliegue automático (Git)" en `Preferences.php`** — mismo patrón colapsable/opcional que "SMTP propio" (ADR-076): `deploy_repo` en texto plano, `deploy_token` enmascarado (nunca se pre-carga en el formulario, "dejar vacío no borra" — mismo criterio que `smtp_password`), y un `Toggle` "Automatización activa" nuevo.
- **El checkbox no se puede tildar sin credenciales completas** — pedido explícito del Tech Lead. `->disabled()` reactivo (`Get $get`, via `->live(onBlur: true)` en ambos campos de texto) más una revalidación server-side en el guardado (`$tenant->deploy_enabled = ... && $tenant->hasDeployWebhookConfigured()`), por si un campo deshabilitado igual dehidrata su último valor conocido.
- **Columna nueva `deploy_enabled`** (boolean, `default(false)`, migración `2026_09_18_070000_add_deploy_enabled_to_tenants_table`) separa dos preguntas que antes eran una sola: `Tenant::hasDeployWebhookConfigured()` (¿hay credenciales?, sin cambios — sigue siendo el gate de `FrontendDeployService::dispatch()`, pensando en un futuro botón manual "Publicar ahora") de la nueva `Tenant::hasAutoDeployActive()` (`deploy_enabled && hasDeployWebhookConfigured()`, el gate REAL que ahora consultan `DeployTriggerObserver`/`TriggerFrontendDeploy`).
- **`deploy_token` pasa a `$fillable`** (antes excluido a propósito — comentario en `Tenant.php` actualizado) — sigue en `$hidden`, sigue siendo una credencial cifrada at-rest, solo cambió QUIÉN puede cargarla.
- **Tests:** `FrontendDeployWebhookTest` gana 2 tests nuevos cubriendo "credenciales cargadas pero automatización apagada" (`test_has_auto_deploy_active_requires_enabled_flag_on_top_of_credentials`, `test_saving_content_with_credentials_but_automation_disabled_does_not_queue_deploy`); `makeTenant(withWebhook: true)` ahora también setea `deploy_enabled: true`, así que los tests existentes siguen pasando sin cambiar su semántica.
- **Verificación:** sin runtime de PHP en este sandbox — balance de llaves/paréntesis/corchetes verificado manualmente en los 6 archivos tocados/nuevos.
- **Archivos:** `database/migrations/2026_09_18_070000_add_deploy_enabled_to_tenants_table.php` (nuevo), `app/Models/Tenant.php`, `app/Filament/Pages/Preferences.php`, `app/Observers/DeployTriggerObserver.php`, `app/Jobs/TriggerFrontendDeploy.php`, `tests/Feature/FrontendDeployWebhookTest.php`. Ver addendum ADR-072.
- **Pendiente:** `php artisan migrate`, `php artisan test --compact --filter=FrontendDeployWebhookTest`, `vendor/bin/pint --dirty --format agent`, y que el Tech Lead confirme en vivo (para CICA360, que ya tenía `deploy_repo`/`deploy_token` seteados por tinker antes de esta migración: el checkbox arranca en `false` por default, así que hay que entrar a Preferencias y tildarlo manualmente una vez para que el mecanismo siga disparando).
- **Corrección inmediata:** el Tech Lead marcó 2 problemas de copy sobre el primer intento — (1) el ejemplo de repositorio usaba un cliente real (`eduflores/cica360`) en vez de un placeholder genérico (Stamless es multi-tenant, ese campo lo ve cualquier cliente); (2) texto en voseo rioplatense en `Preferences.php`. Corregido: ejemplo genérico (`usuario/repositorio`) y las 3 frases nuevas reescritas a construcciones neutras/infinitivas; de paso se corrigieron 2 instancias de voseo PREEXISTENTES en la misma pantalla (sección SMTP, desde ADR-076) — mismo criterio ya documentado en `PageResource.php` desde 2026-09-11 (ADR-051: el voseo es exclusivo de la marca CICA360, nunca de Stamless/Studio).
- **2do pedido de reorden:** mover "SMTP propio (opcional)" y "Despliegue automático (Git)" al final de la página, después de "Open Graph" — ambas son secciones técnicas/de infraestructura, distintas del resto (identidad, cuenta, SEO/OG), tiene sentido agruparlas al final. Se mantienen a 2 columnas cada una (`->columns(2)`, sin `->columnSpanFull()`) — "Open Graph" sigue siendo la única sección a ancho completo de toda la página.

## 2026-09-18 — Fix real de producción: `FormField.options`/`validation_rules` se guardaban como objeto JSON en vez de array (mismo bug que ADR-071, nunca portado a `FormResource`)

- **Reporte del Tech Lead**, tras la Fase 2: captura de `TypeError: (field.options ?? []).map is not a function` en `ContactForm.tsx` sobre el campo "País" — el parche inicial del lado cliente (`fieldIsRenderable()`, ver entrada de más abajo) dejó de crashear pero terminó OCULTANDO el campo "País" del sitio público pese a estar `is_active=true` en Studio, lo cual el Tech Lead marcó correctamente como el síntoma real de un bug de fondo, no el comportamiento esperado.
- **Causa raíz encontrada:** exactamente el mismo bug ya documentado y resuelto para `colophon` en `PageResource::isUuidKeyedArray()`/ADR-071, nunca portado a `FormResource`. `saveRelationshipsUsing()` del `Builder` de "fields" reindexa el NIVEL SUPERIOR (`array_values($state)`) pero los Repeaters ANIDADOS dentro de cada bloque (`options`, `validation_rules`) seguían keyeados por el ID interno de Livewire de cada ítem (string UUID) al guardar — un array PHP con keys no-secuenciales se serializa a JSON como OBJETO, no como ARRAY. El API pública devolvía entonces `field.options` como `{"93f2...": {...}}` en vez de `[{...}]`, y `.map()` del lado cliente reventaba sobre eso.
- **Fix real:** nuevo `FormResource::reindexRepeaterState()` — `array_values()` sobre `options`/`validation_rules` ANTES de persistir, en `saveRelationshipsUsing()`. Corrige el guardado hacia adelante; los `FormField` YA guardados con el bug (como "País"/"Área de interés" de CICA360) necesitan además una corrección de los datos ya persistidos.
- **Migración de datos nueva** (`2026_09_18_060000_reindex_form_field_options_and_validation_rules`): recorre TODOS los `FormField` existentes (`withoutGlobalScopes()`, cualquier tenant) y reindexa con `array_values()` cualquier `options`/`validation_rules` que no sea ya una lista secuencial (`array_is_list()`, PHP 8.1+) — a diferencia del precedente de ADR-071 (que dejó la corrección de datos legados como "reordenar y guardar a mano"), acá se optó por una migración real porque el síntoma (campo entero desaparecido del sitio público) es más grave que un simple reordenamiento visual y no debía depender de que el Tech Lead supiera que tenía que reabrir y regrabar cada campo afectado.
- **El parche del lado cliente (`fieldIsRenderable()`/`fieldOptions()`, cica360) se mantiene** — sigue siendo la defensa correcta contra CUALQUIER dato malformado futuro (de este bug o de cualquier otro), solo que ahora el dato de origen vuelve a ser válido, así que el campo "País" reaparece en el sitio público en cuanto: (a) corra esta migración, o (b) el Tech Lead reabra y regrabe el campo en Studio (dispara el `saveRelationshipsUsing()` ya corregido).
- **Archivos:** `app/Filament/Resources/FormResource.php` (`reindexRepeaterState()` nuevo + aplicado a `options`/`validation_rules`), `database/migrations/2026_09_18_060000_reindex_form_field_options_and_validation_rules.php` (nuevo).
- **Pendiente:** `php artisan migrate` (suma otra vez a la lista ya pendiente de migraciones sin correr este día).

## 2026-09-18 — Forms Fase 2 implementada: wiring dinámico del front (cica360) por `FormFieldTypeEnum` (ver addendum ADR-073)

- **Reporte del Tech Lead, sobre la página "Contacto" real de CICA360:** "no se ha integrado el formulario, acabo de desactivar 2 campos en el admin [y el público sigue mostrando los 7 de siempre]" — confirma el gap dejado abierto a propósito en ADR-073 (Fase 1 solo cubría el builder en Studio; el sitio público seguía ignorando por completo qué formulario/campos eligió el tenant).
- **Prerrequisito encontrado al ir a conectar `getForm()` (cica360):** `PageResource.php` guardaba el `id` interno del `Form` en `content.form_id` — pero el API público (`GET /forms/{slug}`) solo resuelve formularios por slug, igual que Páginas/Posts/Servicios. Con el `id` guardado, cica360 no tenía forma de pedir el formulario elegido. Fix: campo renombrado a `content.form_slug` (`PageResource.php`, `Cliente0ContentSeeder::upsertContactoPage()`), más una migración de DATOS (`2026_09_18_050000_migrate_contact_form_block_id_to_slug`, con `down()` simétrico) que reescribe cualquier bloque `contact_form` YA guardado en producción.
- **`ContactFormBlock.astro` (cica360):** deja de ignorar `content.form_slug` — lo resuelve en build time con `getForm()` (ya existía en `api.ts`, sin caller hasta ahora) y pasa el `Form` completo como prop a `<ContactForm />`. Fallo silencioso a propósito si `getForm()` rechaza (formulario borrado/inactivo/slug huérfano) — loguea, no renderiza el formulario, no rompe el `astro build` completo por un dato inconsistente de UN bloque.
- **`ContactForm.tsx` (cica360), reescritura completa:** de cero props + 7 campos 100% hardcodeados a `form: FormData` + render dinámico de `form.fields` por `FormFieldTypeEnum` (switch sobre los 11 tipos — `text`/`email`/`tel`/`tel_country`/`textarea`/`select`/`checkbox`/`radio`/`number`/`date`; `file`/`hidden` no se renderizan, ver docblock del archivo). Un campo desactivado en Studio ya no llega en el array (`FormController::show()` ya filtraba `is_active=true` server-side desde antes — el gap real era que el front nunca leía el array).
- **Validación de FORMATO también dinámica:** `validateFieldValue()` interpreta `FormFieldData.validation_rules` (mismos strings `min:`/`max:`/`regex:` que ya expone el API, sembrados en `Cliente0ContentSeeder`) en vez de reglas hardcodeadas por nombre de campo — si el Tech Lead cambia una regla en Studio, el cliente la sigue automáticamente. Los patrones `regex:/.../flags` de este proyecto ya estaban en sintaxis compatible con JS (`\p{L}` + flag `u`), se reutilizan tal cual vía `new RegExp()`; un patrón no portable se ignora en silencio (nunca la autoridad real — el backend sigue validando todo desde cero).
- **WhatsApp/`tel_country`:** misma UX de siempre (bandera + código de país, dígitos agrupados de a 3, preselección por IP), ahora AUTOCONTENIDA por campo (selector de país propio) en vez de depender de un campo "País" de negocio separado — necesario porque ya no hay garantía de que ese campo exista o se llame igual en cualquier form/tenant.
- **Verificación:** sin runtime de PHP en este sandbox (sin cambios adicionales sobre los ya verificados manualmente en ADR-076). Del lado cica360 SÍ hay runtime de Node: `npx tsc --noEmit -p tsconfig.json --ignoreDeprecations 6.0` corrió limpio (exit 0) sobre el proyecto completo. `astro check`/`astro sync` fallan en este sandbox puntual por un binding nativo de `rolldown` faltante para esta plataforma (`Cannot find module '@rolldown/binding-wasm32-wasi'`) — ajeno a este cambio, no se intentó "arreglar" reinstalando `node_modules` del proyecto real sin que el Tech Lead lo pida.
- **Archivos:** `app/Filament/Resources/PageResource.php`, `database/seeders/Cliente0ContentSeeder.php`, `database/migrations/2026_09_18_050000_migrate_contact_form_block_id_to_slug.php` (nuevo) — genesis. `src/components/blocks/ContactFormBlock.astro`, `src/components/islands/ContactForm.tsx` (reescrito) — cica360.
- **Pendiente:** `php artisan migrate` (suma a la lista ya pendiente de ADR-076), y una prueba real en Studio — desactivar/reactivar un campo del form "Contacto" y confirmar en el sitio publicado (tras rebuild) que el público lo refleja.

## 2026-09-18 — Investigación abierta: con 2 emails en `notification_email` solo llega 1 al admin, y la "copia al usuario"/constancia no llega

- **Reporte del Tech Lead**, tras el fix del `BindingResolutionException`: guardó `notification_email` con 2 correos separados por coma y `send_copy_to_submitter` activo (captura de la sección "Notificaciones y respuesta") — esperaba 3 emails (to + cc del admin + constancia al remitente) y solo llegó 1 al admin, ninguno de constancia.
- **Sin acceso a producción desde este sandbox** (`studio.stamless.host` es un servidor remoto — no hay SSH, DB ni logs accesibles acá), no se pudo reproducir ni confirmar la causa raíz en vivo. Se auditó el código y el mecanismo interno de Laravel (`vendor/laravel/framework`, instalado localmente) para descartar/confirmar hipótesis:
  - `Mail::to($to)->cc($cc)->send($mailable)` con un Mailable `ShouldQueue` SÍ preserva `to`/`cc` a través del encolado — `PendingMail::fill()` los aplica al Mailable (`$mailable->to()->cc()->bcc()`) ANTES de encolar (`Mailer::sendMailable()` → `$mailable->queue()`), y el worker despacha el mismo Mailable ya poblado (`SendQueuedMailable::handle()` → `$this->mailable->send($factory)`). El código en sí no tiene el bug obvio de "cc se pierde al encolar".
  - `config/mail.php` de este repo no define ningún override `'to'` global (el mecanismo de Laravel `setGlobalToAndRemoveCcAndBcc()`, que SI existiera activo redirigiría todo a una sola dirección y BORRARÍA cc/bcc) — no está configurado acá, aunque no se pudo confirmar que la config real de producción coincida con este repo.
- **Gap real encontrado y corregido:** `notifySubmitter()` retornaba en silencio (sin ningún log) cuando `send_copy_to_submitter` está apagado O cuando `$contact->email` viene vacío — indistinguible entre "el toggle está apagado" y "este envío puntual no tenía email" sin poder inspeccionar la base de datos. Se agregó `Log::debug()` en ambos `notifyAdmin()` (recipients `to`/`cc` antes de despachar) y `notifySubmitter()` (motivo de omisión si aplica, o destinatario antes de despachar) — deja rastro en `storage/logs/laravel.log` para diagnosticar la PRÓXIMA vez que se reproduzca.
- **Hipótesis a descartar por el Tech Lead (checklist), en orden de probabilidad:**
  1. **CC en un inbox separado:** un email con `Cc: correo2@...` NO aparece en la bandeja de `correo1@...` — cada destinatario (to y cc) recibe su propia copia en SU PROPIA bandeja. Si solo se revisó una de las 2 casillas, puede que el otro correo sí haya llegado a la otra.
  2. **`queue:work` corriendo pero sin worker dedicado o caído a mitad de proceso** — pendiente operativo desde ADR-072, nunca confirmado. Revisar `php artisan queue:failed` y la tabla `jobs`/`failed_jobs` en producción.
  3. **`$contact->email` vacío en ESE envío puntual** — si el campo del formulario usado en la prueba no mapea a `email`/`correo`/`correo_electronico` (ver `ContactSubmissionService::splitPayload()`), `notifySubmitter()` corta antes de intentar nada. Confirmar con `Contact::find($id)->email` en tinker.
  4. **Filtro de spam / proveedor SMTP** (Mailtrap sandbox, límites de cuenta, etc.) descartando silenciosamente cc o el 2do email.
- **Archivos:** `app/Services/ContactSubmissionService.php` (solo logging agregado, sin cambio de comportamiento). Balance verificado manualmente (62/62, 248/248, 76/76).
- **Mejora adicional 2026-09-18, pedido del Tech Lead:** "aplicar un pequeño delay por correo o envío para que así también no bannee el proveedor de SMTP... entre 5 a 30 segundos de forma random, así que no sea predecible los envíos". `notifyAdmin()`/`notifySubmitter()` pasan de `Mail::to()->send($mailable)` a `Mail::to()->later($segundos, $mailable)`, con `random_int(5, 30)` calculado independiente por cada uno (nuevo helper `randomMailDelay()`) — ya no salen "pegados" al mismo segundo entre sí ni de forma predecible frente al proveedor SMTP. Mismo mecanismo de cola de siempre (`ShouldQueue` en ambos Mailables), `->later()` solo suma un `available_at` en el futuro — el job no se procesa antes de que venza el delay, el resto del flujo (try/catch, logs) no cambia.
- **CERRADO 2026-09-18, causa real confirmada con logs reales del Tech Lead:** el log de diagnóstico agregado arriba confirmó que AMBOS emails se despachan siempre con el `to`/`cc` correctos (`Despachando notificación de nuevo contacto. {"to":"goncalvez.isaac@gmail.com","cc":["zedu77@gmail.com"]}`) — el código nunca tuvo el bug. Lo que fallaba intermitentemente era la ENTREGA: `ContactFormReceipt` reventó con `TransportException: Connection could not be established with host "127.0.0.1:2525": ... Connection refused` — el relay SMTP local (puerto 2525, típico de Mailpit/Herd Mail) no estaba disponible en ese momento puntual. Explica todo el patrón observado en las pruebas previas (a veces llega 1, a veces el otro, a veces ninguno): es la disponibilidad intermitente del relay local la que varía, no la lógica de negocio, que es determinística y correcta. Sin acción de código — es un problema de entorno local del Tech Lead (confirmar que el mail catcher esté corriendo, o apuntar `MAIL_HOST`/`MAIL_PORT` directo a Mailtrap sin relay intermedio). El job de `ContactFormReceipt` que falló quedó en `failed_jobs` (cola corrida con `--tries=1`) — se reprocesa con `php artisan queue:retry all` una vez estable el relay.

## 2026-09-18 — SMTP propio por tenant, opcional, con fallback al de la plataforma (ver ADR-076)

- **Pregunta directa del Tech Lead**, tras la investigación de los bugs de email de ADR-075: "en el formulario falta la sección de configuración para ingresar los datos para configurar su SMTP favorito y será mejor para evitar usar mi smtp general para todo o que me recomiendas mantener un solo SMTP común para todos mis clientes? comentame". Se presentó un análisis de trade-offs (reputación de dominio/IP compartida, "From" que no coincide con la marca del cliente, límites de volumen de un solo proveedor) recomendando un enfoque híbrido — confirmado por el Tech Lead vía pregunta estructurada ("Sí, híbrido con fallback (recomendado)").
- **Migración + `Tenant`:** 7 columnas nullable nuevas (`smtp_host`/`port`/`username`/`password`/`encryption`/`from_address`/`from_name`) — `smtp_password` con cast `encrypted` (mismo criterio que `deploy_token`) y en `$hidden`, pero SÍ en `$fillable` (a diferencia de `deploy_token`, acá el propio tenant lo carga desde Preferencias, no solo el operador vía tinker). Nuevos helpers: `hasCustomSmtpConfigured()`, `smtpMailerConfig()`, `smtpFromAddress()`.
- **Aplicación en los Mailables — la parte no obvia:** nuevo trait `App\Mail\Concerns\UsesTenantSmtp`, aplicado en `ContactFormSubmitted`/`ContactFormReceipt`, que sobreescribe `Mailable::prepareMailableForDelivery()` — el único hook de Laravel que corre en CADA intento real de envío (síncrono o desde el worker de cola tras deserializar el job), y que se ejecuta ANTES de que `Mailable::send()` resuelva qué mailer usar. Ahí se registra un mailer efímero (`config(['mail.mailers.tenant_smtp_{id}' => ...])`) con las credenciales del tenant leídas FRESCAS de la base de datos en ese momento — necesario porque las relaciones Eloquent no sobreviven la serialización de la cola (`SerializesModels` solo preserva el modelo raíz, no sus relaciones cacheadas; se recargan solas al accederlas).
- **Detalle técnico encontrado al investigar:** Symfony Mailer NO tiene una opción `encryption` — decide TLS/SSL solo por el `scheme` de la Dsn (`smtps` = TLS implícito, `smtp` = STARTTLS automático) o por el puerto (465 → implícito). El viejo `MAIL_ENCRYPTION` de Laravel está deprecado. El select "Cifrado" de Preferencias se traduce a `scheme` en `Tenant::smtpMailerConfig()`, no se pasa como una opción que Symfony ignoraría en silencio.
- **`Preferences.php`:** nueva sección "SMTP propio (opcional)", colapsada por defecto. `smtp_password` nunca se precarga en el formulario (no se manda la contraseña guardada de vuelta al navegador) — helper text avisa si ya hay una guardada, y el guardado solo la sobreescribe si el tenant tipeó una nueva. Botón "Probar conexión" (`headerActions()` de la Section) que manda un EMAIL REAL de prueba al admin logueado, con los datos tal como están tipeados en ese momento (sin guardar primero) — replica el mismo mecanismo interno de `Illuminate\Mail\MailManager::createSmtpTransport()` (`EsmtpTransportFactory` + `Dsn` construidos programáticamente) en vez de armar una DSN a mano con `sprintf()`, evitando bugs de escape con caracteres especiales en usuario/contraseña.
- **Verificación:** sin runtime de PHP en este sandbox — balance de llaves/paréntesis/corchetes verificado manualmente en cada archivo (excluyendo comentarios, con paréntesis sueltos en prosa sin efecto real). Sin tests nuevos; pendiente `php artisan migrate` + suite + `pint` + probar en vivo el botón "Probar conexión" con un SMTP real.
- **Archivos:** `database/migrations/2026_09_18_040000_add_smtp_config_to_tenants_table.php` (nuevo), `app/Models/Tenant.php`, `app/Mail/Concerns/UsesTenantSmtp.php` (nuevo), `app/Mail/ContactFormSubmitted.php`, `app/Mail/ContactFormReceipt.php`, `app/Filament/Pages/Preferences.php`. Ver ADR-076.

## 2026-09-18 — Fix producción: `BindingResolutionException` al editar cualquier Form (regla closure mal formada en `notification_email`) (ver addendum ADR-075)

- **Reporte del Tech Lead:** captura de un error 500 real en `studio.stamless.host` al intentar editar un `Form` existente ("quise editar el formulario") — `BindingResolutionException` en `EvaluatesClosures.php:102`: "An attempt was made to evaluate a closure for [Filament\Forms\Components\TextInput], but [$attribute] was unresolvable". Regresión introducida por mí en la misma sesión, junto con el resto de ADR-075 — rompía la edición de CUALQUIER Form en producción, no solo el guardado.
- **Causa:** la regla de validación custom de `notification_email` (multi-email por coma) se había pasado como `->rules([function (string $attribute, mixed $value, \Closure $fail) {...}])` — un closure crudo directo en el array de `->rules()`. Filament no lo reenvía tal cual al validador de Laravel: evalúa CADA closure de ese array con su propio mecanismo de inyección (`EvaluatesClosures`, el mismo que resuelve `Get $get`/`$record` en cualquier otro campo), y `$attribute` no es un parámetro que sepa resolver — explota antes de validar nada, incluso solo al MONTAR el formulario.
- **Fix:** envolver en un closure exterior sin parámetros (nada que Filament necesite inyectar) que DEVUELVE el closure de validación real — patrón documentado por Filament: `->rules([fn (): \Closure => function (string $attribute, mixed $value, \Closure $fail) {...}])`. El closure exterior es trivial de evaluar; su valor de retorno es el que llega al validador de Laravel con la firma correcta.
- **Verificación:** grep confirmó que era el único `->rules([` con un closure crudo en toda la app — no había más lugares con el mismo bug. Balance de llaves/paréntesis/corchetes de `FormResource.php` verificado manualmente (38/38, 468/468, 57/57) — sin runtime de PHP en este sandbox.
- **Archivos:** `app/Filament/Resources/FormResource.php`. Ver addendum en ADR-075 (`docs/context/DECISIONS.md`).
- **Pendiente:** confirmar en Studio que editar un Form ya no rompe, y que la validación multi-email (coma) sigue funcionando como se espera.

## 2026-09-18 — Emails de notificación con marca del tenant, envío por cola, multi-email y "copia al usuario" real (ver ADR-075)

- **Contexto real, captura de Mailtrap del email de notificación ya en producción:** genérico, header "Stamless" (branding de la plataforma) sin relación al tenant que lo envía. Pedido del Tech Lead en un solo mensaje largo: personalizar textos/título del mailing, logo del tenant en el header (configurable en Preferencias), que sea "plantilla predeterminada para todos los clientes con sus tenants" (no algo por tenant desde cero), `notification_email` con soporte a varios correos separados por coma (primero = to, resto = cc), y "el envío a correo tiene que ser usando queue también" (mensaje aparte, mismo turno).
- **Plantilla HTML propia compartida, reemplaza el markdown genérico de Laravel:** `emails/layouts/branded.blade.php` (header con logo o nombre, footer, un solo lugar para cambiar el diseño a futuro) + `emails/contacts/form-submitted.blade.php`/`form-receipt.blade.php`. Se descartó publicar el theme `mail::message` de Laravel (`vendor:publish`) — ese componente no expone `$header`/`$footer` override-ables desde la vista de contenido sin tocar los archivos vendor; HTML propio con estilos inline da control total sin ese paso.
- **Logo del tenant:** nuevo `Setting` `branding.logo_id` (mismo mecanismo que las imágenes OG, ADR-065), sección "Marca" nueva en `Preferences.php`. `ContactSubmissionService::resolveBrandLogoUrl()` lo resuelve con `withoutGlobalScopes()` + `where('tenant_id', ...)` explícito — el helper `setting()`/scope automático de tenant dependen de `TenantManager::hasTenant()`, que el submit público de la API nunca setea.
- **`notification_email` multi-destinatario:** columna `string`→`text` (migración nueva), validación Filament reemplazada por una regla closure que valida cada segmento separado por coma, `ContactSubmissionService::notifyAdmin()` arma `Mail::to($primero)->cc($resto)`. Bug propio detectado y corregido antes de continuar: `[$to, ...$cc] = $recipients` NO es sintaxis válida de PHP (a diferencia de JS, no hay "rest pattern" en destructuring) — se cambió a `array_shift()`.
- **`notification_intro` nuevo:** campo de texto opcional (ocupa el espacio que dejó libre `success_message`), personaliza la línea introductoria del email al admin.
- **"Enviar copia a quien completa el formulario" implementado de verdad:** columna/toggle (`Form::send_copy_to_submitter`) existían desde el MVP pero nunca se leían en ningún lado — hallazgo de esta vuelta, mismo patrón que `FormField::validation_rules` antes de ADR-054. Nuevo Mailable `App\Mail\ContactFormReceipt`, reusa el mismo contenido de la "Página de Agradecimiento" (`ContactSubmissionService::resolveThankYouData()`, MOVIDO desde `FormSubmissionController` — antes privado ahí, ahora público en el service para compartirlo entre la respuesta HTTP del submit y este email nuevo) en vez de duplicar textos personalizables por `Form` en dos lugares.
- **Envío por cola:** `ContactFormSubmitted`/`ContactFormReceipt` ganan `implements ShouldQueue` — `Mail::to()->send()` lo detecta solo y despacha a la cola (comportamiento nativo de `Illuminate\Mail\Mailer::sendMailable()`), sin tocar ningún call site del service. Cada Mailable define `failed()` para loguear fallos del envío real en el worker (el `try/catch` del service solo cubre el despacho).
- **3 rondas de ajustes de layout en `FormResource.php`, mismo turno, viendo capturas de Studio:**
  1. "Email y asunto, esa fila debería ser a 2 columnas ya, por que al ocultarse el tipo se quedó en base a 3 columnas" — esto en realidad era sobre el builder de campos (`fieldBlockSchema()`), no esta sección: `Hidden::make('type')` seguía contando como 3er hijo del `Grid` aunque no pintara nada. Grid pasa a 2 columnas para campos de catálogo, `Hidden` sale del grid.
  2. "No olvides poner en una sola columna todos los campos de esa sección (Notificaciones y respuesta), y spanfull el check de la sección general" — Toggle "Enviar copia..." y el Toggle "Activo" de la sección "General" ganan `columnSpanFull()`; el bloque Email/Asunto/Intro ya estaba en un `Grid(1)` propio.
  3. "Pasar la sección de seguridad al final del modal, después de la página de agradecimientos, y spanfull, ancho completo" — `Section::make('Seguridad')` se mueve de entre "Notificaciones" y el builder de campos a DESPUÉS de "Página de Agradecimiento" (última sección del form), gana `->columnSpanFull()`.
- **Tono de copy:** corregidos 2 textos con voseo rioplatense (`Podés`/`Creá`/`elegí`) a formas neutras/impersonales en `FormResource.php` — pedido explícito del Tech Lead ("no obligar a leer argento o uruguayo"), y alineado con una regla YA documentada en ADR-051 (voseo es exclusivo de la marca CICA360, Studio/Console siempre en español neutro).
- **Verificación:** sin runtime de PHP en este sandbox — balance de llaves/paréntesis verificado manualmente en cada archivo tocado/nuevo. Sin tests nuevos; pendiente `php artisan migrate`, `php artisan test --compact`, `vendor/bin/pint --dirty --format agent`, validación visual completa y confirmar que `queue:work` esté corriendo (pendiente operativo ya existente desde ADR-072).
- **Archivos:** `app/Services/ContactSubmissionService.php`, `app/Mail/ContactFormSubmitted.php`, `app/Mail/ContactFormReceipt.php` (nuevo), `app/Http/Controllers/Api/V1/FormSubmissionController.php`, `app/Models/Form.php`, `app/Filament/Resources/FormResource.php`, `app/Filament/Pages/Preferences.php`, `resources/views/emails/layouts/branded.blade.php` (nuevo), `resources/views/emails/contacts/form-submitted.blade.php` (reescrito), `resources/views/emails/contacts/form-receipt.blade.php` (nuevo), `database/migrations/2026_09_18_020000_update_forms_notification_columns.php` (nuevo). Ver ADR-075.

## 2026-09-18 — Builder con block-picker para "Agregar campo" + Página de Agradecimiento trasladada a Form (ver ADR-074)

- **Pedido 1 (UX del builder), mid-turno sobre la Fase 1 recién entregada:** "el botón agregar campo que sea similar a agregar bloque de contenido, que sea un dropdown con opciones con icono, más coherente y amigable para los usuarios finales, con buen UX y cada campo con collapsed para aprovechar altura" — con una captura mostrando el Select "Campo del catálogo" (1ra vuelta) dentro de un ítem ya agregado en blanco.
- **`FormResource::form()` — `fields` pasa de `Repeater` a `Builder`:** el picker nativo de `Builder` (mismo componente, mismo `block-picker.blade.php` ya afinado en `PageResource::blocks`) YA es el dropdown buscable con ícono por opción pedido — elegir el tipo de campo pasa a ser el PRIMER paso ("Agregar campo") en vez de un Select dentro de un ítem en blanco. Un `Block` por `FormFieldDefinition` (ícono propio: usuario/sobre/teléfono/edificio/etiqueta/pin de mapa/globo/estrella/burbuja de chat según la clave) + un bloque "Campo personalizado". `->collapsed()` a nivel Builder (antes solo `->collapsible()`) para aprovechar altura, como se pidió. Como `field_definition_id` (no una columna "tipo de bloque") es el discriminador real de cada campo, se necesita `loadStateFromRelationshipsUsing()`/`saveRelationshipsUsing()` custom (mismo patrón que `PageResource::blocks`), resolviendo la clave del bloque elegido contra el catálogo en el momento de guardar (sin pisar la columna `type` real, que sigue siendo el tipo de INPUT del campo — texto/email/tel/...). Misma red de seguridad que `PageResource` (ADR-071 addendum): estado vacío en un form que YA tiene campos aborta el guardado con un log, en vez de borrarlos por un glitch de hidratación. Eager load de `fieldDefinition` en la carga de estado para evitar N+1.
- **Bug real en producción, encontrado al probar:** `BadMethodCallException: Method Filament\Forms\Components\Builder::relationship does not exist.` — a diferencia de `Repeater`, `Builder` en esta versión de Filament NO tiene método `->relationship()` (había sido copiado del primer diseño con `Repeater`, sin verificar contra runtime real por falta de PHP en el sandbox). Confirmado el patrón correcto revisando `PageResource::blocks`: NUNCA llama `->relationship()` tampoco — el par `loadStateFromRelationshipsUsing()`/`saveRelationshipsUsing()` son hooks independientes, no requieren `->relationship()` para funcionar. Se quitó la llamada; el resto del mecanismo (ya escrito con esos 2 hooks) no necesitó cambios.
- **UX/integridad de datos, mismo turno, pedido del Tech Lead viendo el Select de "Tipo" editable en un campo de catálogo ya elegido ("Nombre"):** "los presets deberían incluir el tipo texto predeterminado, por que si no el usuario podría cometer el error de seleccionar tipo email o celular u otro, no es usable... a menos que sea un custom field, ahí sí amerita". El campo `type` pasa a `Hidden` (preseteado, no editable) para los 9 bloques de catálogo — solo el bloque "Campo personalizado" mantiene el Select real. `Hidden` en vez de `->disabled()` a propósito: evita el riesgo ya documentado en las guías de este proyecto de que un campo deshabilitado deje de dehidratar su valor.
- **Mismo pedido, 2da parte — "considerar las validaciones predeterminadas... que ya tenga internamente sus validates... para ahorrar pasos al usuario":** `FormFieldDefinition.validation_rules` (columna que existía en el schema desde el MVP pero NUNCA se poblaba) ahora trae presets reales para `name`/`city`/`email`/`message` — las mismas regex ya probadas en producción por CICA360 (`Cliente0ContentSeeder::upsertContactForm()`), promovidas de "override puntual de un tenant" a "default genérico del catálogo global" porque son genéricas de verdad (nombre = solo letras, email = TLD válido, mensaje = sin HTML).
- **Corrección del Tech Lead sobre lo anterior (4ta vuelta, mismo turno):** "ojo a validates cuando son campos del catálogo predeterminados, entonces debería tener presets fijos... a menos que sean necesarios, pero yo creo que no necesita, internamente se podría setear" — el Repeater "Reglas de validación adicionales" NO queda editable-con-default para campos de catálogo (como quedó en la vuelta anterior): pasa a `Hidden` (mismo criterio que `type`), fijo e interno, sin mostrarse en absoluto. Solo "Campo personalizado" mantiene el Repeater real, porque ahí no hay ningún preset del que partir.
- **Pedido adicional, mismo turno, viendo el campo "Mensaje de éxito":** "este campo creo que ya no va o si es necesario?" — confirmado por grep que `ContactForm.tsx` nunca leyó `success_message` (solo usa el objeto `thank_you` del submit). Se quita del formulario de `FormResource` sin borrar la columna/dato (legado de CICA360, expuesto igual por el API resource por si algún consumidor externo la usa).
- **Pedido adicional, mismo turno:** "los campos de la página de agradecimiento poner al final de los campos del formulario" — la sección "Página de Agradecimiento" pasa de ir ANTES del builder de campos a ser el ÚLTIMO componente del schema.
- **Pedido 2, mensaje aparte (mismo turno):** captura de la sección "Página de Agradecimiento (Formularios)" ya funcionando en Preferencias — "ahora tiene sentido, trasladar el contenido de la página gracias que sea dinámica, implementé esto en preferencias y hay que trasladarlo ahora al formulario para personalizar los textos ahí mismo, y actualizar la integración del gracias en cica360".
- **Los 5 campos (`thank_you_title`/`description`/`alert_title`/`alert_description`/`button_label`) se trasladan de `Setting` tenant-wide a columnas de `Form`:** migración nueva, `Form::casts()` gana `'thank_you_description' => 'array'` (el `RichEditor` de Filament guarda JSON, a diferencia de `Setting.value` que era `text` sin cast — mismatch que no se hereda). Sección idéntica (mismos labels/helpers) ahora vive en `FormResource`, colapsada por defecto. `Preferences.php` pierde la sección + su wiring de fill/save.
- **API:** `Http\Resources\Api\V1\FormResource` (`forms/{slug}`) y `FormSubmissionController::resolveThankYouData()` (respuesta de `submit`) leen `$form->thank_you_*` en vez de `setting('thank_you.*')`. De paso, los defaults hardcodeados de fallback se vuelven genéricos (ya no mencionan "CICA360" en código compartido por todos los tenants — un problema latente preexistente). `store()` ahora resuelve el `Form` ANTES del chequeo de honeypot (antes ese camino no tenía ningún `Form` disponible) para que hasta la respuesta falsa-exitosa a un bot muestre el mensaje del formulario correcto, no un genérico desconectado.
- **`Cliente0ContentSeeder::upsertContactForm()`** siembra los 5 campos con el copy real ya usado por CICA360, dejando ese form 100% autocontenido (no depende de ningún fallback de código ni de Preferencias).
- **cica360: sin cambios de código.** Verificado `ContactForm.tsx`/`types.ts` (`ThankYouTemplate`): el contrato JSON del objeto `thank_you` es idéntico antes/después — solo cambió de dónde el backend saca los valores.
- **Verificación:** sin runtime de PHP en este sandbox — balance de llaves/paréntesis/corchetes revisado en los 7 archivos tocados (limpio). Los 2 tests de `FormSubmissionApiTest` que ya cubrían la forma de `thank_you` no aseveran el texto exacto de los defaults que cambiaron, así que siguen aplicando sin tocar. Pendiente: `php artisan migrate`, `php artisan test --compact`, `vendor/bin/pint --dirty --format agent`, y probar visualmente el nuevo picker + la sección de Agradecimiento en Studio.
- **Archivos:** `app/Filament/Resources/FormResource.php`, `app/Filament/Pages/Preferences.php`, `app/Models/Form.php`, `app/Http/Resources/Api/V1/FormResource.php`, `app/Http/Controllers/Api/V1/FormSubmissionController.php`, `database/migrations/2026_09_18_010000_add_thank_you_fields_to_forms_table.php` (nuevo), `database/seeders/Cliente0ContentSeeder.php`, `database/seeders/FormFieldDefinitionSeeder.php`, `docs/context/DECISIONS.md` (ADR-074).
- **Siguiente:** el Tech Lead corre migración + suite + pint en el servidor, prueba el picker de campos y la sección de Agradecimiento en Studio, y — si quiere que el "WhatsApp" ya sembrado tome el copy nuevo — confirma que `db:seed` corrió sobre el tenant CICA360 real.

## 2026-09-18 — Formularios por tenant, Fase 1: `FormResource` en Studio (ver ADR-073)

- **Pedido del Tech Lead:** "el detalle de servicios, los testimonios, los sliders, el formulario se necesita que sea por tenant y no se donde se crea el nombre de formulario principal o contacto principal que no lo encuentro donde cambiar en el admin, el usuario debería poder crear el formulario a manera de build, indicando de la lista de inventario de campos que campo va usar para su formulario y así poder decidir que formulario usa o cambia... primero dime que plan tienes para resolver esto? tiene que tener mucho UX esto." Plan de 3 fases presentado y confirmado con 2 preguntas de `AskUserQuestion` (campo WhatsApp → "tipo reutilizable"; catálogo de `FormFieldDefinition` → "fijo por ahora"), luego **"nada, procede"**.
- **Diagnóstico previo:** `Form`/`FormField` ya eran modelos completos y multi-tenant desde el MVP (`Form` vía `HasTenant`, `FormField` sin `tenant_id` propio pero resuelto vía accessor a `form.tenant_id`), sembrados por `Cliente0ContentSeeder::upsertContactForm()` y consumidos por `forms/{slug}` + el Select `content.form_id` de `PageResource`. El gap real era 100% de UI: cero pantallas en Studio para verlos/editarlos, de ahí la confusión reportada.
- **`FormResource.php` (nuevo):** grupo de nav "Clientes" (junto a `ContactResource`, dado que un Form es la fuente de los Contacts). Form principal: nombre/slug (auto-slug + `scopedUnique` por tenant, mismo patrón que `SliderResource`)/descripción/activo; sección colapsable de notificaciones (email/asunto/copia al remitente/mensaje de éxito/URL de redirección); sección colapsable de seguridad (honeypot/reCAPTCHA). Builder de campos: `Repeater::make('fields')->relationship('fields')->orderColumn('sort_order')` (mismo patrón que `SliderResource::slides`), cada ítem con un Select opcional de `FormFieldDefinition` del catálogo (autocompleta label/name/type/is_required/is_encrypted vía `Set` en `afterStateUpdated`, sigue siendo editable después) o en blanco para un campo 100% a medida. Campos editables por ítem: etiqueta (con auto-slug a "nombre técnico" si está vacío), nombre técnico (`->distinct()` para unicidad dentro del form), tipo (`FormFieldTypeEnum`), placeholder, texto de ayuda, obligatorio, cifrado, opciones (Repeater `{value,label}`, visible solo para Select/Radio) y reglas de validación avanzadas (`Repeater::simple()` — array plano de strings, evita el problema de un `TagsInput` con separador de coma partiendo regex que traen comas, como la regla de "Consulta" de CICA360).
- **`Tenant::maxForms()`:** Free/Freemium=2, Auspicio/Convenio=5 — mismo criterio que `maxSliders()` (un Form es una pieza que se arma, no un ítem de contenido suelto). Wired con el patrón estándar `isFormLimitReached()`/`formLimitMessage()`/badge de nav (`FormatsUsageBadge`).
- **`FormFieldTypeEnum::TelCountry`** (`'tel_country'`, "Teléfono con selector de país") — generaliza el WhatsApp+país+detección-por-IP de CICA360 (hoy 100% hardcodeado en `ContactForm.tsx`) a una opción de catálogo disponible para cualquier tenant, decisión ya confirmada por el Tech Lead. La `FormFieldDefinition` global `phone` pasa de `Tel` a `TelCountry` en el seeder — re-sembrar actualiza el catálogo; el `FormField` "WhatsApp" ya sembrado de CICA360 toma el tipo nuevo recién cuando `Cliente0ContentSeeder::upsertContactForm()` se vuelva a correr (queda como pendiente operativo, no autoejecutado).
- **Fuera de alcance de esta fase, a propósito:** `ContactFormBlock.astro` (cica360) sigue ignorando `content.form_id` por completo — el sitio público sigue mostrando el `ContactForm.tsx` hardcodeado hasta Fase 2 (wiring del front, no iniciada). Tampoco se abrió un CRUD del catálogo `FormFieldDefinition` (decisión explícita del Tech Lead).
- **Verificación:** sin runtime de PHP en este sandbox — revisión manual línea por línea de balance de llaves/paréntesis/corchetes e imports en los 4 archivos nuevos/tocados. Sin tests nuevos en esta pasada (pendiente para el cierre real de la Fase 1, ver ADR-073 → Consecuencias).
- **Archivos:** `app/Enums/FormFieldTypeEnum.php`, `app/Models/Tenant.php` (`maxForms()`), `database/seeders/FormFieldDefinitionSeeder.php`, `app/Filament/Resources/FormResource.php` (nuevo), `app/Filament/Resources/FormResource/Pages/ManageForms.php` (nuevo), `docs/context/DECISIONS.md` (ADR-073).
- **Siguiente:** correr `php artisan test --compact`, `vendor/bin/pint --dirty --format agent` y validar visualmente el builder en Studio (crear un Form de prueba, agregar 2-3 campos del catálogo + 1 a medida, confirmar que el reorden por drag persiste). Si CICA360 debe adoptar `TelCountry` en su "WhatsApp" ya sembrado, correr `php artisan db:seed --class=FormFieldDefinitionSeeder` y re-correr `Cliente0ContentSeeder::upsertContactForm()`. Fase 2 (wiring del front) queda para una sesión propia, sin iniciar acá.

## 2026-09-18 — Fix real del deploy webhook en producción: trait conflict + cobertura de observers (ver ADR-072, actualización)

- **Pedido del Tech Lead:** al probar el mecanismo de la Fase 6 en producción, Studio tiraba `FatalError` al guardar cualquier página. Resuelto ese bloqueo, después reportó: "guarde cambios en contenidos tipo footer en el admin y nunca se ejecutó el envio a git del aviso. asegurate de que para cualquier contenido, preferencia u otra cosa que se necesite actualizar en frontsite [dispare el deploy]".
- **Bug 1 — `PHP Fatal error` de composición de traits (bloqueaba TODO guardado, no solo footer):** `TriggerFrontendDeploy::$queue` redeclaraba una propiedad ya declarada por `Illuminate\Bus\Queueable` (`public $queue;`, sin default). PHP exige firma IDÉNTICA (incluido el default, no solo el tipo) entre una propiedad de clase y la del trait — un primer intento de fix (quitarle el tipo `string`) no alcanzó, porque el default `= 'deploys'` por sí solo ya cuenta como firma distinta. Diagnosticado en producción con `php artisan tinker` (sin OPcache de por medio en CLI, descartando caché como causa) tras confirmar archivo correcto en disco, FPM reiniciado y vendor fresco (`composer install`) — el error persistía igual, aislando el problema al lenguaje, no a ningún tipo de caché. Fix: no redeclarar la propiedad, asignar `$this->queue = 'deploys';` en el constructor.
- **Ajuste:** `DEBOUNCE_SECONDS` baja de 90 a 30 (pedido del Tech Lead — la espera se sentía larga; el debounce no domina el tiempo total, el build+deploy de GitHub Actions sí, ~2-3 min).
- **Bug 2 — cobertura incompleta de `DeployTriggerObserver` (el reporte real del footer):** auditoría completa de `app/Models/*.php` vs el observer. `Block` (contenido de TODOS los bloques de página, colophon/footer incluido) se había quedado afuera del registro original en 9 modelos — y aunque estuviera en `Page`, guardar solo un bloque vía Repeater no deja "dirty" nada de la Page padre, por lo que ni el evento `updated` de Page se dispara necesariamente. Se agregó el observer a `Block`, `Form` (público vía `forms/{slug}`) y `Media` (público vía `media/{uuid}`, `alt_text` queda horneado en HTML estático). `FormField` no tiene `tenant_id` propio — se resuelve vía un accessor `tenantId()` que lee `form.tenant_id`, sin tocar el observer genérico. Quedaron afuera a propósito: `Contact`/`ContactActivity` (CRM interno), `Domain` (DNS/routing, no contenido), `FormFieldDefinition` (catálogo global sin tenant), billing/plans/modules (fuera del MVP).
- **Deploy a producción (cica360), 3 problemas propios del pipeline de GitHub Actions sin relación al webhook:** Node 20 del workflow vs Astro exigiendo `>=22.12.0` (sube a Node 22); el hosting exige FTPS explícito (`protocol: ftps`) y bloquea `.htaccess` por FTP (`550 Permission denied`, se excluye del deploy automático); `server-dir` apuntaba a la raíz de la cuenta FTP en vez de `public_html/` (duplicó todo el sitio fuera del document root, limpiado a mano desde el File Manager). Con los 3 corregidos, el primer deploy automático end-to-end completo (Genesis → Job → GitHub Actions → build → FTP) corrió exitoso.
- **Tests nuevos:** `FrontendDeployWebhookTest` gana 3 casos — guardado directo de `Block` (sin tocar la Page padre), borrado de `Block`, y guardado de `FormField` (confirma que el accessor resuelve el tenant correcto). Sin runtime de PHP en este sandbox — pendiente correr `php artisan test --compact --filter=FrontendDeployWebhookTest` y `vendor/bin/pint --dirty` en el servidor.
- **Archivos:** `app/Jobs/TriggerFrontendDeploy.php`, `app/Models/{Block,Form,FormField,Media}.php`, `tests/Feature/FrontendDeployWebhookTest.php`, `docs/context/DECISIONS.md` (ADR-072, actualización 2026-09-18).
- **Siguiente:** el Tech Lead corre la suite de tests y `pint` en el servidor, confirma que guardar un bloque de footer/colophon dispara el deploy, y sigue de cerca los próximos guardados de contenido "real" (no solo de prueba) para confirmar que no quedó ningún otro modelo de contenido público afuera.

## 2026-09-17 — Fase 6 (post-MVP) adelantada: deploy webhook por tenant (ver ADR-072)
- **Pedido del Tech Lead:** tras confirmar en producción que un cambio de contenido en Studio (agregó una "X" al título de la Home) no se reflejaba en la website de CICA360 (sitio SSG, hosting cPanel sin Node), planteó el problema de fondo: "de nada sirvió implementar en Astro cuando se busca que el contenido sea dinámico... por eso tiene desde el api las properties que son dinámicas para que se puedan personalizar en cualquier momento sin necesidad de volver a hacer build". Se le presentaron 3 estrategias (fetch en cliente/0 delay, webhook con rebuild automático, híbrido) — eligió la de webhook, priorizando mantener SEO/performance intactos por sobre latencia cero (acepta el delay de ~1-3 min de build+deploy).
- **Implementación (genesis):**
  1. Migración `add_deploy_webhook_fields_to_tenants_table`: `deploy_repo`/`deploy_token` (nullable, opt-in por tenant) en `tenants`.
  2. `Tenant.php`: `deploy_token` cifrado (`encrypted` cast) + fuera de `$fillable`/dentro de `$hidden` (credencial de infraestructura, solo seteable vía `forceFill()`/tinker); nuevo helper `hasDeployWebhookConfigured()`.
  3. `app/Services/FrontendDeployService.php` (nuevo): hace `POST /repos/{deploy_repo}/dispatches` a la API de GitHub con `event_type: content-updated`, usando `deploy_token` como Bearer. No-op si el tenant no tiene ambos campos configurados.
  4. `app/Jobs/TriggerFrontendDeploy.php` (nuevo): `ShouldQueue` + `ShouldBeUnique` (clave por `tenant_id`) — el debounce real: mientras haya un job pendiente/corriendo para ese tenant, nuevos `dispatch()` se descartan en silencio. Delay de 90s aplicado al encolar, `$uniqueFor` 150s (cubre el delay + margen), 3 reintentos con backoff [30, 120, 300]s.
  5. `app/Observers/DeployTriggerObserver.php` (nuevo): observer genérico (`created`/`updated`/`deleted`), registrado en `Page`, `Post`, `Service`, `Slider`, `Slide`, `Menu`, `MenuItem`, `Testimonial`, `Setting` — verifica `hasDeployWebhookConfigured()` antes de encolar, para no ensuciar la tabla `jobs` de tenants sin el mecanismo activado.
  6. Tests nuevos: `tests/Feature/FrontendDeployWebhookTest.php` — cubre el gate de `hasDeployWebhookConfigured()`, que el observer NO encola sin webhook configurado y SÍ lo hace (con el `tenantId` correcto) con webhook configurado, y que `FrontendDeployService` arma el request HTTP correcto (`Http::fake()`) o no hace nada si no corresponde.
- **Implementación (cica360, ver también su propio PROGRESS.md):**
  1. `.github/workflows/build-and-deploy.yml`: nuevo trigger `repository_dispatch` (`types: [content-updated]`, debe coincidir exacto con `FrontendDeployService::EVENT_TYPE`); el paso de deploy FTP (`SamKirkland/FTP-Deploy-Action`, antes comentado/"no en uso") pasa a correr siempre. El artifact `cica360-dist` se mantiene como respaldo/rollback manual.
  2. `docs/context/ARCHITECTURE.md` §6: diagrama y texto actualizados para reflejar el flujo automático (reemplaza la decisión de deploy manual del 2026-09-13).
- **Pendiente operativo (no es código, es configuración del Tech Lead):** (a) setear `deploy_repo`/`deploy_token` del tenant CICA360 en genesis (vía tinker — `deploy_token` nunca se comitea); (b) configurar los secrets `FTP_SERVER`/`FTP_USERNAME`/`FTP_PASSWORD` en GitHub (Settings → Secrets and variables → Actions del repo cica360); (c) confirmar que un worker de colas esté corriendo en producción, escuchando AMBAS colas: `php artisan queue:work --queue=default,deploys` (ver actualización de aislamiento multi-tenant, abajo) — sin eso, los jobs delayed quedan en la tabla `jobs` sin procesarse nunca, y el mecanismo entero queda inerte en silencio.
- **Actualización, mismo día (aislamiento de blast-radius multi-tenant):** el Tech Lead marcó un criterio general para todo el proyecto de acá en más: "siempre considerar que lo que hagamos podría impactar a otros clientes dentro de Stamless, porque el Studio será usado por muchos clientes y proyectos". Revisando el mecanismo bajo ese criterio: 2 ajustes de aislamiento, sin cambiar comportamiento observable. (a) `TriggerFrontendDeploy::$queue = 'deploys'` — este Job es hoy el ÚNICO encolado de todo Genesis; sin cola propia compartiría `default` con cualquier job que se agregue a futuro para otros tenants, y un `deploy_repo` lento/caído de un tenant podría demorar trabajo ajeno. (b) `FrontendDeployService::dispatch()` gana `->timeout(10)` explícito (el default de Laravel es 30s) — evita que un worker quede colgado esperando a GitHub y bloqueando la cola compartida. El resto del diseño (gate por tenant, debounce, migración nullable/opt-in) ya cumplía el criterio desde el vamos — ver ADR-072 (actualización).
- **Verificación:** sin runtime de PHP en este sandbox — balance de llaves/paréntesis/corchetes verificado vía script Python en los 13 archivos PHP tocados/nuevos (sin desbalances nuevos respecto a `git show HEAD`); `pint --dirty` y la suite de tests quedan pendientes de correr por el Tech Lead (no hay PHP disponible acá).
- **Archivos:** `database/migrations/2026_09_17_162311_add_deploy_webhook_fields_to_tenants_table.php`, `app/Models/Tenant.php`, `app/Services/FrontendDeployService.php` (nuevo), `app/Jobs/TriggerFrontendDeploy.php` (nuevo), `app/Observers/DeployTriggerObserver.php` (nuevo), `app/Models/{Page,Post,Service,Slider,Slide,Menu,MenuItem,Testimonial,Setting}.php`, `tests/Feature/FrontendDeployWebhookTest.php` (nuevo), `docs/context/DECISIONS.md` (ADR-072).
- **Siguiente:** el Tech Lead completa los 3 pendientes operativos de arriba, corre `php artisan migrate`, `vendor/bin/pint --dirty --format agent` y `php artisan test --compact --filter=FrontendDeployWebhookTest`, y confirma que guardar contenido de CICA360 en Studio efectivamente dispara un nuevo build+deploy en GitHub Actions.

## 2026-09-17 — Fix real: orden de columnas/sub-bloques de `colophon` se pierde al guardar (ver ADR-071)
- **Pedido del Tech Lead:** "las columnas se me cambiaron de orden al guardar cambios, si intento reordenarlo no se queda ahi, se mantiene el orden de creacion en descendente. deberia mantenerse al order asc" — reportado sobre el bloque `colophon` (columnas del pie de página) en una página tipo Footer, justo después del fix de `MaxNestingDepthExceededException` en el mismo bloque.
- **Investigación (subagente + verificación propia):** causa raíz confirmada en `saveRelationshipsUsing()` del Builder de `blocks` (`PageResource.php`) — el `$state` recibido ahí es el estado CRUDO de Livewire (`getState()`/`getRawState()`, sin pasar por `dehydrateState()`), donde cada ítem de Repeater/Builder queda keyeado por su ID interno de Livewire (string UUID), no por posición. El nivel superior (`blocks`, los bloques de la página) ya se reindexaba correctamente con `array_values($state)` — pero los arrays ANIDADOS dentro de `content` (columnas de `colophon`, sub-bloques `link_list`/`social_links`/`image_link` de cada columna, `data.items` de `social_links`) solo pasaban por `unwrapFileUploadState()`, que recorre recursivamente la estructura pero nunca reindexaba esos arrays. Un array con keys no-secuenciales se serializa a JSON como OBJETO en vez de ARRAY, y Postgres `jsonb` NO garantiza preservar el orden de keys de un objeto al guardarlo — reordena según su propio criterio interno. El síntoma reportado era exactamente ese reordenamiento silencioso de Postgres.
- **Evidencia de que el equipo ya conocía el síntoma del lado de lectura:** `ResolvesPublicLinks::transformBlockContent()` ya hacía `array_values()` defensivo sobre `columns`/`blocks`/`items` de `colophon` al armar la respuesta pública de la API, con un comentario explícito sobre este mismo riesgo — pero ese parche solo cubría la LECTURA (lo que ve el sitio público), nunca la ESCRITURA (lo que Studio guarda), que es donde el usuario reportó el bug real.
- **Fix:** `unwrapFileUploadState()` (`PageResource.php`) gana un paso final — si, tras su recorrido recursivo existente, el array tiene TODAS sus keys en formato UUID (nuevo helper `isUuidKeyedArray()`), se reindexa con `array_values()`. Cubre automáticamente `content.columns` y cualquier nivel de sub-bloques/ítems anidados dentro, sin importar la profundidad, y sin tocar los 2 call-sites existentes (guardado normal del Builder + acción "Copiar bloque a otra página").
- **Verificación:** sin runtime de PHP en este sandbox — balance de llaves/paréntesis/corchetes comparado contra `git show HEAD:app/Filament/Resources/PageResource.php` (script Python), mismo diff pre-existente de paréntesis en comentarios de prosa, sin desbalances nuevos.
- **Importante — dato legado:** columnas ya guardadas con el orden roto (antes de este fix) necesitan un reordenamiento manual más + un guardado nuevo para que el fix las reindexe correctamente; no hay migración retroactiva de datos.
- **Archivos:** `app/Filament/Resources/PageResource.php`, `docs/context/DECISIONS.md` (ADR-071).
- **Siguiente:** pedirle al Tech Lead que reordene las columnas del `colophon` afectado y guarde de nuevo, confirmando que el orden se mantiene tanto en Studio como en el sitio público.

## 2026-09-17 — Fix real: `MaxNestingDepthExceededException` al agregar redes sociales en `colophon` (bloque Footer)
- **Pedido del Tech Lead:** reportó un Internal Server Error al intentar agregar una nueva red social (ej. TikTok, 4to ítem) dentro del sub-bloque "Redes sociales" del bloque `colophon`, en una página tipo Footer.
- **Causa real:** `Livewire\Exceptions\MaxNestingDepthExceededException` — la property path del payload de Livewire (`mountedActions.0.data.blocks.{uuid}.data.content.columns.{uuid}.blocks.{uuid}.data.items.{uuid}`) tiene 14 niveles reales de profundidad, por encima del límite default de Livewire (`payload.max_nesting_depth = 10`, definido en `vendor/livewire/livewire/config/livewire.php`, nunca publicado a nivel de app). Es estructural, no un bug de datos: el Builder de bloques de página → bloque `colophon` → Repeater de `content.columns` → Builder anidado de sub-bloques (`link_list`/`social_links`/`image_link`) → Repeater de `data.items` de `social_links` son 4 niveles de Builder/Repeater anidados, necesarios para el esquema actual del footer multi-columna.
- **Fix:** se publica `config/livewire.php` a nivel de app (antes solo existía el default embebido en el vendor) con **un solo cambio real**: `payload.max_nesting_depth` sube de 10 a 24 — margen suficiente para esta estructura y para otras combinaciones de anidamiento ya existentes en el proyecto (ej. menús de hasta 3 niveles, `MenuTreeBuilder`/ADR-045), sin desactivar la guarda de Livewire por completo.
- **Sin runtime de PHP en este sandbox:** balance de paréntesis/corchetes/llaves verificado vía script Python — sin discrepancias. **El Tech Lead debe correr `php artisan config:clear` (y, si tiene config cacheada con `config:cache`, volver a cachear) para que el cambio tome efecto**, y reintentar agregar la red social.
- **Archivos:** `config/livewire.php` (nuevo).
- **Siguiente:** confirmar con el Tech Lead que, tras `config:clear`, puede agregar/guardar redes sociales sin el error 500. Si en el futuro alguna otra estructura de contenido necesita más profundidad todavía, subir este mismo valor en vez de rediseñar el esquema salvo que el Tech Lead lo pida.
