...
- **Efecto colateral correcto, no buscado a propósito:** el ejemplo de `GET /media/{uuid}` (`"url": "https://cdn.stamless.com/cica360/logo.png"`) también se personaliza — como las URLs reales de CDN sí incluyen el slug del tenant como segmento de path, el ejemplo queda MÁS preciso para cada tenant, no solo consistente.
